                     ...
                     tf_listener_(std::make_shared<tf2_ros::TransformListener>(*tf_buffer_))
{
    ...
    this->declare_parameter<double>("safety_margin", 0.0);

    // Declare cost weights as parameters
    this->declare_parameter<double>("cost_weights.w_s", 10.0);
    this->declare_parameter<double>("cost_weights.w_beta", 5.0);
    this->declare_parameter<double>("cost_weights.w_dsteering", 5.0);
    this->declare_parameter<double>("cost_weights.w_dthrottle", 5.0);
    this->declare_parameter<double>("cost_weights.w_vx", 5.0);
    this->declare_parameter<double>("cost_weights.w_vy", 1.0);
    this->declare_parameter<double>("cost_weights.w_n", 10.0);
    this->declare_parameter<double>("cost_weights.w_u", 1.0);

    // Retrieve cost weights
    double w_s = this->get_parameter("cost_weights.w_s").as_double();
    double w_beta = this->get_parameter("cost_weights.w_beta").as_double(); 
    double w_dsteering = this->get_parameter("cost_weights.w_dsteering").as_double();
    double w_dthrottle = this->get_parameter("cost_weights.w_dthrottle").as_double();
    double w_vx = this->get_parameter("cost_weights.w_vx").as_double();
    double w_vy = this->get_parameter("cost_weights.w_vy").as_double();
    ...
    // Set Parameters
    double p[9] = {w_s, w_beta, w_dsteering, w_dthrottle, w_vx, w_vy, w_n, w_u, safety_margin};
    int idxs[9] = {0, 1, 2, 3, 4, 5, 6, 7}; // indices for the parameters

    for (int i = 0; i < MPC_MODEL_N; i++)
    {
        ocp_nlp_in_set_params_sparse(nlp_config_, nlp_dims_, nlp_in_, i, idxs, p, 9);
    }
    ...
    /* std::vector<double> ubx = {v_x_ref, 0.4, 0.3}; // vx, delta, T
    for (size_t k = 0; k < MPC_MODEL_N; k++)
    {
    ...
    n_x = MPC_MODEL_NX; // Number of states
    n_u = MPC_MODEL_NU; // Number of controls
...
void MPCNode::initMPC()
{
    ...
    {
        RCLCPP_INFO(this->get_logger(), "Initial state: s=%.3f, n=%.3f, u=%.3f, vx=%.3f, vy=%.3f, r=%.3f, delta=%.3f, T=%.3f",
                    current_state_.s, current_state_.n, current_state_.u, current_state_.vx,
                    current_state_.vy, current_state_.r, current_state_.delta, current_state_.T);
        timer_->cancel();

        current_state_.vx = 1.01; // Start with 1 m/s
        Eigen::VectorXd x0(8);
        x0 << current_state_.s, current_state_.n, current_state_.u, current_state_.vx, current_state_.vy, current_state_.r, current_state_.delta, current_state_.T;
        apply_warm_start(x0);
...
void MPCNode::solveMPC()
{
    ...
    state_array_msg.data = current_state_vector_;
    ...
    // Get control output
    std::array<double, MPC_MODEL_NX> state_output_1;
    std::array<double, MPC_MODEL_NU> control_output_1;
    double solved_time;
    double steering_cmd, throttle_cmd;
    ...
    else
    {
        ...
        throttle_cmd = control_output_1[1];
    ...
    control_vesc_msg.drive.acceleration = throttle_cmd;
    ...
    control_array_msg.data = {steering_cmd, throttle_cmd};
    ...
    // Create a vector of MPC_MODEL_NX vectors, each with MPC_MODEL_N+1 elements
    std::vector<std::vector<double>> predict_vector_x(MPC_MODEL_NX, std::vector<double>(MPC_MODEL_N + 1));
    for (int ii = 0; ii <= MPC_MODEL_N; ii++)
    {
        std::array<double, MPC_MODEL_NX> state_vector;
        ocp_nlp_out_get(nlp_config_, nlp_dims_, nlp_out_, ii, "x", state_vector.data());
        // Fill predict_vector_x: [x, y, yaw, theta, v]
        for (int j = 0; j < MPC_MODEL_NX; j++)
        {
            predict_vector_x[j][ii] = state_vector[j];
        }
    }

    std::vector<std_msgs::msg::Float64MultiArray> state_vector_msgs(MPC_MODEL_NX);
    for (int i = 0; i < MPC_MODEL_NX; i++)
    {
...
void MPCNode::apply_warm_start(const Eigen::VectorXd &x)
{
    ...
    auto ws = this->get_warm_start(x, /*const_steer_vel=*/0.0, /*d_acc=*/0.1);
    ...
    double const_steer_vel)
{
    ...
    int state_dim = MPC_MODEL_NX + MPC_MODEL_NU; 

    std::vector<Eigen::VectorXd> warm_start(horizon, Eigen::VectorXd::Zero(state_dim));

    // inicialização (i = 0)
    // Fill the first n_x elements with the state vector x, and the next n_u elements with control guesses
    for (int i = 0; i < this->n_x; ++i)
        warm_start[0](i) = x(i);

    // Fill the next n_u elements with control guesses (const_steer_vel, d_acc)
    if (this->n_u >= 2) {
        warm_start[0](this->n_x + 0) = const_steer_vel;
        warm_start[0](this->n_x + 1) = d_acc;
    }
    // If more control inputs, set to zero or extend as needed
    for (int i = 2; i < this->n_u; ++i)
        warm_start[0](this->n_x + i) = 0.0;
