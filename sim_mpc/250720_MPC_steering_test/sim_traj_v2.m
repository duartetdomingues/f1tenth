...
for t_idx = 1:T_total/Ts-1
   ...
   % 1. Find the closest point to the current position
    ...
    for k = 0:N-1
           yref_k = [x_ref_step(k+1); y_ref_step(k+1); sin(psi_ref(k+1));cos(psi_ref(k+1));0; v_ref(k+1)];
...
psi = x(3); % Orientação no mapa atual
delta = x(4);           % ângulo atual do servo

delta_error = delta_cmd - delta;

max_delta_change = servo_max_rate * Ts;
...
% Usar delta médio para atualizar orientação
delta_avg = (delta + delta_new) / 2;

x_sim = x + Ts * [
    v * cos(psi);        % dx/dt
    v * sin(psi);        % dy/dt
    (v / L) * tan(delta_avg); % dyaw/dt
    0                     % dtheta/dt
