...
#define S_FUNCTION_NAME solverTV_simulinkBlockcompact
...
static void mdlInitializeSizes(SimStruct *S)
{
	...
	/* Output Port 0 */
    ssSetOutputPortMatrixDimensions(S,  0, 145, 1);
...
static void mdlOutputs(SimStruct *S, int_T tid)
{
	...
	params.solver_timeout = (solverTV_float) solver_timeout[0];
    ...
    for(i = 0; i < 10; i++)
	{
		outputs[i] = (real_T) output.zI[i];
	}

	for(i = 0; i < 130; i++)
	{
		outputs[10 + i] = (real_T) output.z[i];
	}

	for(i = 0; i < 5; i++)
	{
		outputs[140 + i] = (real_T) output.zN[i];
	}
