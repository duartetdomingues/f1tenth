...
a Python wrapper for a fast solver generated by FORCESPRO v6.3.0
   ...
   OUTPUT = solverTV_py.solverTV_solve(PARAMS) solves a multistage problem
   subject to the parameters supplied in the following dictionary:
       PARAMS['xinit'] - matrix of size [4x1]
       PARAMS['x0'] - matrix of size [145x1]
       PARAMS['all_parameters'] - matrix of size [630x1]
       PARAMS['solver_timeout'] - scalar
       PARAMS['num_of_threads'] - scalar

   OUTPUT returns the values of the last iteration of the solver where
       OUTPUT['zI'] - column vector of length 10
       OUTPUT['z'] - column vector of length 130
       OUTPUT['zN'] - column vector of length 5

   [OUTPUT, EXITFLAG] = solverTV_py.solverTV_solve(PARAMS) returns additionally
   ...
   [OUTPUT, EXITFLAG, INFO] = solverTV_py.solverTV_solve(PARAMS) returns 
...
try:
    _lib = ctypes.CDLL(os.path.join(os.path.dirname(os.path.abspath(__file__)),'solverTV/lib/solverTV_withModel.so'))
    csolver = getattr(_lib,'solverTV_solve')
except:
    _lib = ctypes.CDLL(os.path.join(os.path.dirname(os.path.abspath(__file__)),'solverTV/lib/libsolverTV_withModel.so'))
    csolver = getattr(_lib,'solverTV_solve')

internal_mem = getattr(_lib,'solverTV_internal_mem')

class solverTV_params_ctypes(ctypes.Structure):
#    @classmethod
#    def from_param(self):
#        return self
    _fields_ = [('xinit', ctypes.c_double * 4),
('x0', ctypes.c_double * 145),
('all_parameters', ctypes.c_double * 630),
('solver_timeout', ctypes.c_double),
('num_of_threads', ctypes.c_uint),
]

solverTV_params = {'xinit' : np.array([]),
'x0' : np.array([]),
...
solverTV_params_types = {'xinit' : np.float64,
'x0' : np.float64,
...
class solverTV_outputs_ctypes(ctypes.Structure):
#    @classmethod
#    def from_param(self):
#        return self
    _fields_ = [('zI', ctypes.c_double * 10),
('z', ctypes.c_double * 130),
('zN', ctypes.c_double * 5),
]

solverTV_outputs = {'zI' : np.array([]),
'z' : np.array([]),
'zN' : np.array([]),
}


class solverTV_info(ctypes.Structure):
#    @classmethod
#    def from_param(self):
#        return self
...
class solverTV_mem(ctypes.Structure):
    pass
...
csolver.argtypes = ( ctypes.POINTER(solverTV_params_ctypes), ctypes.POINTER(solverTV_outputs_ctypes), ctypes.POINTER(solverTV_info), ctypes.POINTER(solverTV_mem), ctypes.POINTER(FILE))
csolver.restype = ctypes.c_int

internal_mem.argtypes = [ctypes.c_uint]
internal_mem.restype = ctypes.POINTER(solverTV_mem)

def solverTV_solve(params_arg):
    '''
a Python wrapper for a fast solver generated by FORCESPRO v6.3.0
 ...
 See also COPYING
    ...
    # convert parameters
    params_py = solverTV_params_ctypes()
    for par in params_arg:
        ...
        try:
            ...
            if isinstance(getattr(params_py, par), ctypes.Array):
                params_arg[par] = np.require(params_arg[par], dtype=solverTV_params_types[par], requirements='F')
    ...
    outputs_py = solverTV_outputs_ctypes()
    info_py = solverTV_info()
    if sys.version_info.major == 2:
        ...
        try:
            PyFile_AsFile.restype = ctypes.POINTER(FILE)
            exitflag = _lib.solverTV_solve( ctypes.byref(params_py), ctypes.byref(outputs_py), ctypes.byref(info_py), internal_mem(0), PyFile_AsFile(fp) , _lib.solverTV_adtool2forces )
        ...
        try:
            if sys.platform.startswith('win'):
                exitflag = _lib.solverTV_solve( ctypes.byref(params_py), ctypes.byref(outputs_py), ctypes.byref(info_py), internal_mem(0), None , _lib.solverTV_adtool2forces)
            else:
                exitflag = _lib.solverTV_solve( ctypes.byref(params_py), ctypes.byref(outputs_py), ctypes.byref(info_py), internal_mem(0), fp , _lib.solverTV_adtool2forces)
            libc.fclose(fp)
    ...
    # convert outputs
    for out in solverTV_outputs:
        solverTV_outputs[out] = npct.as_array(getattr(outputs_py,out))

    return solverTV_outputs,int(exitflag),info_py

solve = solverTV_solve
