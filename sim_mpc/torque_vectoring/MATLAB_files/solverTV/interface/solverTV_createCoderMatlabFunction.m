...
% 
% solverTV_CREATECODERMATLABFUNCTION(MODELNAME, BLOCKNAME)
%
% This function generates a Simulink Model with Fixed-step solver type named MODELNAME to add the solverTV FORCESPRO solver as 
% a MATLAB Function Simulink Block named BLOCKNAME so that it can be used for Simulink simulation and 
% Simulink Code Generation.
%
% solverTV_CREATECODERMATLABFUNCTION(MODELNAME, BLOCKNAME, USECOMPACTBLOCK)
% can be used to generate a MATLAB Function Simulink Block with compact inputs and outputs.
%
% solverTV_CREATECODERMATLABFUNCTION(MODELNAME, BLOCKNAME, USECOMPACTBLOCK, USEEXISTINGMODEL)
% can be used if the selected Simulink Model already exists in order to add the Simulink Block to it.
%
% solverTV_CREATECODERMATLABFUNCTION(MODELNAME, BLOCKNAME, USECOMPACTBLOCK, USEEXISTINGMODEL, OPENSIMULINKMODEL)
% can be used to select whether to open the simulink model.
function solverTV_createCoderMatlabFunction(modelname, blockname, useCompactBlock, useExistingModel, openSimulinkModel)

    if nargin < 1 || isempty(modelname)
        modelname = 'solverTV_model';
    end
    if ~isvarname(modelname)
        error('Modelname must be a valid variable name');
    end
    if nargin < 2 || isempty(blockname)
        blockname = 'solverTV_block';
    end
    ...
    function_name = 'solverTV_coderFunction.m';
    position = [170, 99, 650, 265];
    parameter_sizes = struct('xinit', '[4,1]',...
                             'x0', '[145,1]',...
                             'all_parameters', '[630,1]',...
                             'solver_timeout', '[1,1]',...
                             'num_of_threads', '[1,1]');
    if useCompactBlock
        function_name = 'solverTV_coderFunctioncompact.m';
        position = [170, 99, 650, 265];
        parameter_sizes = struct('xinit', '[4,1]',...
                                 'x0', '[145,1]',...
                                 'all_parameters', '[630,1]',...
