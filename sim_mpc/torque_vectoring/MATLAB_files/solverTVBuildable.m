...
% 
classdef solverTVBuildable < coder.ExternalDependency
    ...
    methods (Static)
        ...
        function updateBuildInfo(buildInfo, cfg)
            ...
            if(~strcmp(foldername, 'interface') || ~strcmp(solvername, 'solverTV'))
                solverpath = fullfile(buildablepath, 'solverTV');
            end
            solverInfo = struct();
            solverInfo.solvername = 'solverTV';
        ...
        function [output, exitflag, info] = forcesInitOutputsMatlab()
            ...
            outputs_zI = coder.nullcopy(zeros(10, 1));
            outputs_z = coder.nullcopy(zeros(130, 1));
            outputs_zN = coder.nullcopy(zeros(5, 1));
            output = struct('zI', outputs_zI,...
                            'z', outputs_z,...
                            'zN', outputs_zN);
        ...
        function [output,exitflag,info] = forcesCallWithParams(params)
            [output,exitflag,info] = solverTVBuildable.forcesCall(params.xinit, params.x0, params.all_parameters, params.solver_timeout, params.num_of_threads);
        end
        ...
        function [output, exitflag, info] = forcesCall(xinit, x0, all_parameters, solver_timeout, num_of_threads)
            ...
            [output_c, exitflag_c, info_c] = solverTVBuildable.forcesInitOutputsC(); %#ok<ASGLU>
            ...
            [output, exitflag, info] = solverTVBuildable.forcesInitOutputsMatlab();
            ...
            output.zI = cast(output_c.zI, 'like', output.zI);
    ...
    methods (Static, Access = private)
        function [output, exitflag, info] = forcesInitOutputsC()
            ...
            outputs_zI = coder.nullcopy(double(zeros(10, 1)));
            outputs_z = coder.nullcopy(double(zeros(130, 1)));
            outputs_zN = coder.nullcopy(double(zeros(5, 1)));
            output = struct('zI', outputs_zI,...
                            'z', outputs_z,...
                            'zN', outputs_zN);
