...
class MPCSim:
    ...
    def run(self, sim_time: float) -> SimResult:
        ...
        print("Starting sim loop, total steps:", n_steps)
        ...
        for k in range(n_steps):
            ...
            if (self.x[0] // self.s_ref[-1] != self.lap_counter):
                ...
                if self.lap_counter == 1:
                    ...
                    self.xN = self.xN[-2:]
                    self.uN = self.uN[-2:]
                    self.alpha_f = self.alpha_f[-2:]
                    self.alpha_r = self.alpha_r[-2:]
                    self.Fz_f = self.Fz_f[-2:]
                    self.Fz_r = self.Fz_r[-2:]
                    self.Fx_f = self.Fx_f[-2:]
                    self.Fx_r = self.Fx_r[-2:]
                    self.Fy_f = self.Fy_f[-2:]
                    self.Fy_r = self.Fy_r[-2:]
                    status_hist = status_hist[-2:]
                    solve_ms = solve_ms[-2:]
...
def main():
    ...
    ap.add_argument("--sim-time", type=float, default=600.0, help="Total sim time [s]")
    ap.add_argument(
        "--track-width",
        type=float,
        default=None,
