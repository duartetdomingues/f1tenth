...
sys.path.append(os.path.abspath(".."))  # ou o caminho correto
import time
import numpy as np
...
os.chdir("/home/duarte/Desktop/f1tenth")  # define o diretório de trabalho
    ...
    def run(self, sim_time: float) -> SimResult:
        ...
        for k in range(n_steps):
            ...
            alpha_f, alpha_r = self.model.alpha_func(
                self.x[StateIndex.VELOCITY_V_X],
                self.x[StateIndex.VELOCITY_V_Y],
                self.x[StateIndex.YAW_RATE],
                self.x[StateIndex.DELTA],
            )
            ...
            # detect lap completion safely (need at least two history points)
            if self.x[0] // self.s_ref[-1] != self.lap_counter:
                self.lap_counter = self.x[0] // self.s_ref[-1]
                print("Lap completed!")
                if self.lap_counter == 1:
                    print("First lap completed, resetting simulation.")
                    x_hist = x_hist[-1:]
                    u_hist = u_hist[-1:]
                    self.xN = self.xN[-1:]
                    self.uN = self.uN[-1:]
                    self.alpha_f = self.alpha_f[-1:]
                    self.alpha_r = self.alpha_r[-1:]
                    self.Fz_f = self.Fz_f[-1:]
                    self.Fz_r = self.Fz_r[-1:]
                    self.Fx_f = self.Fx_f[-1:]
                    self.Fx_r = self.Fx_r[-1:]
                    self.Fy_f = self.Fy_f[-1:]
                    self.Fy_r = self.Fy_r[-1:]
                    status_hist = status_hist[-1:]
                    solve_ms = solve_ms[-1:]
                elif self.lap_counter > 1:
                    print("Multiple laps completed, ending simulation.")
                    save = True
                    if save:
                        timestamp = time.strftime("%Y%m%d_%H%M%S")
                        fname = f"lap_data_{timestamp}_mpc_curv_off_py.npz"
                        np.savez(
                            fname,
                            x_hist=np.array(x_hist),
                            u_hist=np.array(u_hist),
                            xN=np.array(self.xN, dtype=object),
                            uN=np.array(self.uN, dtype=object),
                            alpha_f=np.array(self.alpha_f),
                            alpha_r=np.array(self.alpha_r),
                            Fz_f=np.array(self.Fz_f),
                            Fz_r=np.array(self.Fz_r),
                            Fx_f=np.array(self.Fx_f),
                            Fx_r=np.array(self.Fx_r),
                            Fy_f=np.array(self.Fy_f),
                            Fy_r=np.array(self.Fy_r),
                        )
                        print(f"Lap data saved to {fname}.")
                    break

            if status != 0:
                signal.signal(
                    signal.SIGINT, handler_plots
                )  # agora Ctrl+C chama handler
                if sys.stdin.isatty():
                    ...
                    print(
                        "Non-interactive session detected, continuing after solver fallback."
                    )
...
def main():
    ...
    traj_default = "./traj/centerline_0.20_test_map.csv"
    ...
    x_init = np.array([30.1, 0.1, 0.01, 2.0, 0.01, 0.01, 0.0, 0.1], dtype=float)
    ...
    ap.add_argument(
        "--mpc_config",
        type=Path,
        default=Path("./sim_mpc/250916_MPC_curv_off_ls_py/mpc_config.yaml"),
        help="YAML-like MPC params file",
    )
    ap.add_argument(
        "--car_config",
        type=Path,
        default=Path("./sim_mpc/250916_MPC_curv_off_ls_py/car_model.yaml"),
        help="YAML-like car params file",
    )
    ap.add_argument("--sim-time", type=float, default=50.0, help="Total sim time [s]")
    ap.add_argument(
        "--track-width",
        type=float,
        default=None,
    ...
    if any("--f=" in arg for arg in sys.argv):
        args = ap.parse_args(args=[])  # ignora args do kernel
    else:
        args = ap.parse_args()  # comportamento normal no terminal
    ...
    plot_states(
        res.mpc_self, res.x_hist, res.u_hist, res.dt, save_prefix=None, show=True
    )
