...
def make_configs(overrides: dict | None = None) -> tuple[SimpleNamespace, SimpleNamespace, SimpleNamespace]:
    ...
    # slack penalties
    stmpc.Zl = float(cfg["Zl"]) 
    stmpc.Zu = float(cfg["Zu"]) 
    stmpc.zl = float(cfg["zl"]) 
    stmpc.zu = float(cfg["zu"])

    # weights
    stmpc.qs = float(cfg["qs"])      # progress maximization
    stmpc.qbeta = float(cfg["qbeta"]) # side slip angle smoothing
    stmpc.qdthrottle = float(cfg["qdthrottle"]) # throttle rate smoothing
    stmpc.qddelta = float(cfg["qddelta"])     # steering rate smoothing
    stmpc.qvx = float(cfg.get("qvx"))      # longitudinal velocity maximization
    stmpc.qvy = float(cfg.get("qvy"))
    stmpc.qn = float(cfg.get("qn"))        # lateral position minimization
    stmpc.qtheta = float(cfg.get("qtheta"))  # heading angle minimization

    # bounds for states & inputs
    stmpc.v_min = float(cfg["v_min"]) 
    stmpc.v_max = float(cfg["v_max"]) 
    stmpc.throttle_min = float(cfg["throttle_min"]) 
    stmpc.throttle_max = float(cfg["throttle_max"]) 
    stmpc.delta_min = float(cfg["delta_min"]) 
    stmpc.delta_max = float(cfg["delta_max"]) 
    stmpc.ddelta_min = float(cfg["ddelta_min"]) 
    stmpc.ddelta_max = float(cfg["ddelta_max"]) 
    stmpc.dthrottle_min = float(cfg["dthrottle_min"]) 
    stmpc.dthrottle_max = float(cfg["dthrottle_max"]) 
