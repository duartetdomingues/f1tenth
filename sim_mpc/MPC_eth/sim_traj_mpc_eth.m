...
if(compile_solver || ~exist('solver', 'var') )
    clear solver
    json = 'acados_ocp.json';  % ajusta o caminho/nome
    opts = struct( ...
        'json_file', json, ...
        'build', false, ...              % não compilar a shared lib
        'generate', false, ...           % não gerar C novamente
        'compile_mex_wrapper', false, ...% não recompilar o mex wrapper
        'compile_interface', false, ...  % não compilar interface
        'output_dir', fullfile(pwd, 'c_generated_code') );
    
    solver = AcadosOcpSolver([], opts);
...
%    -0.0492
%     0.1000];
x =[27;0.1;0.1;0.1;0.1;0.0;0.0;0.0 ];
