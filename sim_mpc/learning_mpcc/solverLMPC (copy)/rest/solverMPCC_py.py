...
a Python wrapper for a fast solver generated by FORCESPRO v6.2.0
   ...
   OUTPUT = solverMPCC_py.solverMPCC_solve(PARAMS) solves a multistage problem
   subject to the parameters supplied in the following dictionary:
       PARAMS['xinit'] - matrix of size [14x1]
       PARAMS['x0'] - matrix of size [485x1]
       PARAMS['all_parameters'] - matrix of size [63302x1]
       PARAMS['solver_timeout'] - scalar
       PARAMS['num_of_threads'] - scalar

   OUTPUT returns the values of the last iteration of the solver where
       OUTPUT['z'] - column vector of length 464
       OUTPUT['zN_1'] - column vector of length 14
       OUTPUT['zN'] - column vector of length 7

   [OUTPUT, EXITFLAG] = solverMPCC_py.solverMPCC_solve(PARAMS) returns additionally
   ...
   [OUTPUT, EXITFLAG, INFO] = solverMPCC_py.solverMPCC_solve(PARAMS) returns 
...
try:
    _lib = ctypes.CDLL(os.path.join(os.path.dirname(os.path.abspath(__file__)),'solverMPCC/lib/solverMPCC_withModel.so'))
    csolver = getattr(_lib,'solverMPCC_solve')
except:
    _lib = ctypes.CDLL(os.path.join(os.path.dirname(os.path.abspath(__file__)),'solverMPCC/lib/libsolverMPCC_withModel.so'))
    csolver = getattr(_lib,'solverMPCC_solve')

internal_mem = getattr(_lib,'solverMPCC_internal_mem')

class solverMPCC_params_ctypes(ctypes.Structure):
#    @classmethod
#    def from_param(self):
#        return self
    _fields_ = [('xinit', ctypes.c_double * 14),
('x0', ctypes.c_double * 485),
('all_parameters', ctypes.c_double * 63302),
('solver_timeout', ctypes.c_double),
('num_of_threads', ctypes.c_uint),
]

solverMPCC_params = {'xinit' : np.array([]),
'x0' : np.array([]),
'all_parameters' : np.array([]),
'solver_timeout' : np.array([]),
...
solverMPCC_params_types = {'xinit' : np.float64,
'x0' : np.float64,
'all_parameters' : np.float64,
'solver_timeout' : np.float64,
'num_of_threads' : np.uint32,
}

class solverMPCC_outputs_ctypes(ctypes.Structure):
#    @classmethod
#    def from_param(self):
#        return self
    _fields_ = [('z', ctypes.c_double * 464),
('zN_1', ctypes.c_double * 14),
('zN', ctypes.c_double * 7),
]

solverMPCC_outputs = {'z' : np.array([]),
'zN_1' : np.array([]),
'zN' : np.array([]),
}


class solverMPCC_info(ctypes.Structure):
#    @classmethod
#    def from_param(self):
#        return self
...
class solverMPCC_mem(ctypes.Structure):
    pass
...
csolver.argtypes = ( ctypes.POINTER(solverMPCC_params_ctypes), ctypes.POINTER(solverMPCC_outputs_ctypes), ctypes.POINTER(solverMPCC_info), ctypes.POINTER(solverMPCC_mem), ctypes.POINTER(FILE))
csolver.restype = ctypes.c_int

internal_mem.argtypes = [ctypes.c_uint]
internal_mem.restype = ctypes.POINTER(solverMPCC_mem)

def solverMPCC_solve(params_arg):
    '''
a Python wrapper for a fast solver generated by FORCESPRO v6.2.0
 ...
 See also COPYING
    ...
    # convert parameters
    params_py = solverMPCC_params_ctypes()
    for par in params_arg:
        ...
        try:
            ...
            if isinstance(getattr(params_py, par), ctypes.Array):
                params_arg[par] = np.require(params_arg[par], dtype=solverMPCC_params_types[par], requirements='F')
    ...
    outputs_py = solverMPCC_outputs_ctypes()
    info_py = solverMPCC_info()
    if sys.version_info.major == 2:
        ...
        try:
            PyFile_AsFile.restype = ctypes.POINTER(FILE)
            exitflag = _lib.solverMPCC_solve( ctypes.byref(params_py), ctypes.byref(outputs_py), ctypes.byref(info_py), internal_mem(0), PyFile_AsFile(fp) , _lib.solverMPCC_adtool2forces )
        ...
        try:
            if sys.platform.startswith('win'):
                exitflag = _lib.solverMPCC_solve( ctypes.byref(params_py), ctypes.byref(outputs_py), ctypes.byref(info_py), internal_mem(0), None , _lib.solverMPCC_adtool2forces)
            else:
                exitflag = _lib.solverMPCC_solve( ctypes.byref(params_py), ctypes.byref(outputs_py), ctypes.byref(info_py), internal_mem(0), fp , _lib.solverMPCC_adtool2forces)
            libc.fclose(fp)
    ...
    # convert outputs
    for out in solverMPCC_outputs:
        solverMPCC_outputs[out] = npct.as_array(getattr(outputs_py,out))

    return solverMPCC_outputs,int(exitflag),info_py

solve = solverMPCC_solve
