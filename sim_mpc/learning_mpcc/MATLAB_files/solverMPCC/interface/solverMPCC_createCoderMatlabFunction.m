...
% 
% solverMPCC_CREATECODERMATLABFUNCTION(MODELNAME, BLOCKNAME)
%
% This function generates a Simulink Model with Fixed-step solver type named MODELNAME to add the solverMPCC FORCESPRO solver as 
% a MATLAB Function Simulink Block named BLOCKNAME so that it can be used for Simulink simulation and 
% Simulink Code Generation.
%
% solverMPCC_CREATECODERMATLABFUNCTION(MODELNAME, BLOCKNAME, USECOMPACTBLOCK)
% can be used to generate a MATLAB Function Simulink Block with compact inputs and outputs.
%
% solverMPCC_CREATECODERMATLABFUNCTION(MODELNAME, BLOCKNAME, USECOMPACTBLOCK, USEEXISTINGMODEL)
% can be used if the selected Simulink Model already exists in order to add the Simulink Block to it.
%
% solverMPCC_CREATECODERMATLABFUNCTION(MODELNAME, BLOCKNAME, USECOMPACTBLOCK, USEEXISTINGMODEL, OPENSIMULINKMODEL)
% can be used to select whether to open the simulink model.
function solverMPCC_createCoderMatlabFunction(modelname, blockname, useCompactBlock, useExistingModel, openSimulinkModel)

    if nargin < 1 || isempty(modelname)
        modelname = 'solverMPCC_model';
    end
    if ~isvarname(modelname)
        error('Modelname must be a valid variable name');
    end
    if nargin < 2 || isempty(blockname)
        blockname = 'solverMPCC_block';
    end
    ...
    function_name = 'solverMPCC_coderFunction.m';
    position = [170, 99, 650, 265];
    parameter_sizes = struct('x0', '[859,1]',...
                             'xinit', '[14,1]',...
                             'all_parameters', '[21471,1]',...
                             'solver_timeout', '[1,1]',...
                             'num_of_threads', '[1,1]');
    if useCompactBlock
        function_name = 'solverMPCC_coderFunctioncompact.m';
        position = [170, 99, 650, 265];
        parameter_sizes = struct('x0', '[859,1]',...
                                 'xinit', '[14,1]',...
                                 'all_parameters', '[21471,1]',...
