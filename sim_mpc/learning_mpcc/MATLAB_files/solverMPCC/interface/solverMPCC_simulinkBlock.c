...
#define S_FUNCTION_NAME solverMPCC_simulinkBlock
...
#include "../include/solverMPCC_memory.h" 
...
extern solver_int32_default solverMPCC_adtool2forces(double *x, double *y, double *l, double *p, double *f, double *nabla_f, double *c, double *nabla_c, double *h, double *nabla_h, double *hess, solver_int32_default stage, solver_int32_default iteration, solver_int32_default threadID);
solverMPCC_extfunc pt2function_solverMPCC = &solverMPCC_adtool2forces;
...
static void mdlInitializeSizes(SimStruct *S)
{
	...
	/* Input Port 0 */
    ssSetInputPortMatrixDimensions(S,  0, 859, 1);
	...
	/* Input Port 1 */
    ssSetInputPortMatrixDimensions(S,  1, 14, 1);
	...
	/* Input Port 2 */
    ssSetInputPortMatrixDimensions(S,  2, 21471, 1);
	...
	/* Output Port 0 */
    ssSetOutputPortMatrixDimensions(S,  0, 833, 1);
...
static void mdlOutputs(SimStruct *S, int_T tid)
{
	...
	const real_T *solver_timeout = (const real_T*) ssGetInputPortSignal(S,3);
	const real_T *num_of_threads = (const real_T*) ssGetInputPortSignal(S,4);
	
    real_T *z = (real_T*) ssGetOutputPortSignal(S,0);
	real_T *zN_1 = (real_T*) ssGetOutputPortSignal(S,1);
	real_T *zN = (real_T*) ssGetOutputPortSignal(S,2);
	

    /* Solver data
     * Note: mem struct may store a state in case of warm-starting */
    static solverMPCC_params params;
    static solverMPCC_output output;
    static solverMPCC_info info;
    static solverMPCC_mem * mem;
	...
	params.solver_timeout = (solverMPCC_float) *solver_timeout;

	params.num_of_threads = (solver_int32_unsigned) *num_of_threads;

	

    #if SET_PRINTLEVEL_solverMPCC > 0
    ...
    if (mem == NULL)
    {
        mem = solverMPCC_internal_mem(0);
    }

    /* Call solver */
    solver_exitflag = solverMPCC_solve(&params, &output, &info, mem, fp , pt2function_solverMPCC);
	...
	for(i = 0; i < 15; i++)
	{
		zN_1[i] = (real_T) output.zN_1[i];
	}

	for(i = 0; i < 11; i++)
	{
		zN[i] = (real_T) output.zN[i];
	}
