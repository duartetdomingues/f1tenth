function formulateMPCC_agnostic_NN_soft_friction(store_mex_data,differentailCoeffs) %TO DO - ADD ELLIPSE CONSTRAINT, add cost term regarding the slip angle in order to reduce aggressiveness (AMZ), use the derivative of the control input, incorporate lift force(downforce), add cone related constraints, look into the warm start, check for inequalities that depend on consecutive timesteps, might be necessary to take the last ones, tire models need a closer look, add the jacobian (might speed up), experiment with different solvers to see which one performs better, decouple the throttle from the problem and send that command to a PID controller like pure pursuit did
    ...
    % State Transition
    model.eq{N-1} = @(z,p) [progress_dynamics(z(9),z(12),z(13),z(14),p); vehicle_dynamics(z(10:15),z(2:5),[p; z(6:8)])];
...
function dX = dynamic_bicycle_model(x,u,p)
    ...
    a_x = get_ax(u(3),u(4),x(5),x(6),F_roll,F_drag,p);
    ...
    Fx_fl = eta_motor*T_fl*GR/r_wheel*u(1);
    Fx_fr = eta_motor*T_fr*GR/r_wheel*u(1);
    Fx_rl = eta_motor*T_rl*GR/r_wheel*u(1);
    Fx_rr = eta_motor*T_rr*GR/r_wheel*u(1);
...
function h = ineqM(z,p)
    ...
    a_x = get_ax(throttle_prev,steering_prev,vy,r,F_roll,F_drag,p);
    % ay
    a_y = get_ay(vx,r);
    ...
    h = [
         d_throttle/delta_throttle_max;       % rate throttle
         d_steering/delta_steering_max;       % rate steering
         (a_x/mu_x)^2 + (a_y/mu_y)^2 - (g+accel_downforce)^2 - epsilon       % friction circle
         ];  
end
...
function a_x = get_ax(throttle,steering,vy,r,F_roll,F_drag,p)
    % Parameters
    m = p(12);
    T_max_front = p(14);
    T_max_rear = p(15);
    ...
    a_x = (F_f_ax*cos(steering) + F_r_ax - F_roll - F_drag)/m + vy*r;
end

function a_y = get_ay(vx,r)
    a_y = vx*r;
end
%% ----------- Exit Flags -----------------------------------------------
