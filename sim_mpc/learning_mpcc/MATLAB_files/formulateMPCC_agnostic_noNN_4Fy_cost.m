function formulateMPCC_agnostic_noNN_4Fy_cost(store_mex_data,differentailCoeffs) %TO DO - ADD ELLIPSE CONSTRAINT, add cost term regarding the slip angle in order to reduce aggressiveness (AMZ), use the derivative of the control input, incorporate lift force(downforce), add cone related constraints, look into the warm start, check for inequalities that depend on consecutive timesteps, might be necessary to take the last ones, tire models need a closer look, add the jacobian (might speed up), experiment with different solvers to see which one performs better, decouple the throttle from the problem and send that command to a PID controller like pure pursuit did
    ...
    % State Transition
    model.eq{1} = @(z,p) [euler(z(3),z(1)); euler(z(4),z(2)); z(3); z(4); z(14); z(15); z(16); progress_dynamics(z(10),z(13),z(14),z(15),p); vehicle_dynamics(z(11:16),z(3:6),[p; z(7:9)])];
    ...
    for i=2:N-2
        ...
        % State Transition
        model.eq{i} = @(z,p) [euler(z(3),z(1)); euler(z(4),z(2)); z(3); z(4); z(14); z(15); z(16); progress_dynamics(z(10),z(13),z(14),z(15),p); vehicle_dynamics(z(11:16),z(3:6),[p; z(7:9)])];
        if i==N-2
    ...
    % State Transition
    model.eq{N-1} = @(z,p) [progress_dynamics(z(8),z(11),z(12),z(13),p); vehicle_dynamics(z(9:14),z(1:4),[p; z(5:7)])];
...
function f = objectiveMiddle(z,p)
    ...
    % Drag Force
    F_drag = 1/2*rho*C_d*vx^2;

    % Load transfers
    [a_x, a_y] = get_accels(throttle_prev,steering_prev,vx,vy,r,F_roll,F_drag,p);

    f = q_r * ((a_x^2 + a_y^2) / (mu*g)^2) + alpha_CL*e_CL^2 + alpha_L*e_L^2 + beta_steer*d_steering^2 + beta_throttle*d_throttle^2 + exp(q_v_max*(vel-v_max)) - lambda*vk*integrator_stepsize;
end
%N-1
function f = objectiveN_1(z,p)
   ...
   % Parameters
    ...
    f = alpha_CL*e_CL^2 + alpha_L*e_L^2 + exp(q_v_max*(vel-v_max)) - lambda*vk*integrator_stepsize;
end
% End
...
function dX = dynamic_bicycle_model(x,u,p)
    ...
    % Load transfers
    [a_x, a_y] = get_accels(u(3),u(4),x(4),x(5),x(6),F_roll,F_drag,p);
    ...
    alpha_fl = u(2) - (x(5) + x(6)*l_f) / (x(4) - x(6)*track_width/2);
    alpha_fr = u(2) - (x(5) + x(6)*l_f) / (x(4) + x(6)*track_width/2);
    alpha_rl = (x(5) - x(6)*l_r) / (x(4) - x(6)*track_width/2);
    alpha_rr = (x(5) - x(6)*l_r) / (x(4) + x(6)*track_width/2);

    % Static + Downforce + Longitudinal Load Transfer
    Fz_fl = Ff_z_static + Ff_downforce - delta_Fz_x - delta_Fz_y;
    Fz_fr = Ff_z_static + Ff_downforce - delta_Fz_x + delta_Fz_y;
    Fz_rl = Fr_z_static + Fr_downforce + delta_Fz_x - delta_Fz_y;
    Fz_rr = Fr_z_static + Fr_downforce + delta_Fz_x + delta_Fz_y;
    ...
    M_z = (F_f*sin(u(2)) + Ff_tire*cos(u(2)))*l_f + Fr_tire*l_r + diff_Mz + track_width/2*(Fy_fl-Fy_fr)*sin(u(2));
...
function [diff_front, diff_rear] = differential(v_x,delta,a_x,diff_gain)
    ...
    diff_rear = diff_T*(0.5+ratio);
    diff_front = diff_T*(0.5-ratio)/cos(delta);
end

function [a_x, a_y] = get_accels(throttle,steering,vx,vy,r,F_roll,F_drag,p)
    % Parameters
    GR = p(18);
    r_wheel = p(20);
    eta_motor = p(19);
    ...
    % a_x
    F_f_ax = 2*eta_motor*T_front*GR/r_wheel*throttle;
    F_r_ax = 2*eta_motor*T_rear*GR/r_wheel*throttle;
    a_x = ((F_f_ax*steering) + F_r_ax - F_roll - F_drag)/m - vy*r;
