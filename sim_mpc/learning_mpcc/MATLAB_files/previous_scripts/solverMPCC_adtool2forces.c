								 ...
								 solver_int32_default threadID   /* Id of caller thread                                */)
{
    ...
    solverMPCC_callback_float w[1204];
    ...
    solverMPCC_callback_float nabla_c_sparse[49];
	...
	if ((stage >= 34) && (stage < 35))
	{
		out[0] = &this_f;
		out[1] = nabla_f_sparse;
		solverMPCC_objective_35(in, out, NULL, w, 0);
		if (nabla_f != NULL)
		{
			nrow = solverMPCC_objective_35_sparsity_out(1)[0];
			ncol = solverMPCC_objective_35_sparsity_out(1)[1];
			colind = solverMPCC_objective_35_sparsity_out(1) + 2;
			row = solverMPCC_objective_35_sparsity_out(1) + 2 + (ncol + 1);
			solverMPCC_sparse2fullcopy(nrow, ncol, colind, row, nabla_f_sparse, nabla_f);
		}

		out[0] = c_sparse;
		out[1] = nabla_c_sparse;
		solverMPCC_dynamics_35(in, out, NULL, w, 0);
		if (c != NULL)
		{
			nrow = solverMPCC_dynamics_35_sparsity_out(0)[0];
			ncol = solverMPCC_dynamics_35_sparsity_out(0)[1];
			colind = solverMPCC_dynamics_35_sparsity_out(0) + 2;
			row = solverMPCC_dynamics_35_sparsity_out(0) + 2 + (ncol + 1);
			solverMPCC_sparse2fullcopy(nrow, ncol, colind, row, c_sparse, c);
		}

		if (nabla_c != NULL)
		{
			nrow = solverMPCC_dynamics_35_sparsity_out(1)[0];
			ncol = solverMPCC_dynamics_35_sparsity_out(1)[1];
			colind = solverMPCC_dynamics_35_sparsity_out(1) + 2;
			row = solverMPCC_dynamics_35_sparsity_out(1) + 2 + (ncol + 1);
			solverMPCC_sparse2fullcopy(nrow, ncol, colind, row, nabla_c_sparse, nabla_c);
		}

	}

	if ((stage >= 35) && (stage < 36))
	{
		out[0] = &this_f;
		out[1] = nabla_f_sparse;
		solverMPCC_objective_36(in, out, NULL, w, 0);
		if (nabla_f != NULL)
		{
			nrow = solverMPCC_objective_36_sparsity_out(1)[0];
			ncol = solverMPCC_objective_36_sparsity_out(1)[1];
			colind = solverMPCC_objective_36_sparsity_out(1) + 2;
			row = solverMPCC_objective_36_sparsity_out(1) + 2 + (ncol + 1);
			solverMPCC_sparse2fullcopy(nrow, ncol, colind, row, nabla_f_sparse, nabla_f);
		}
