function formulateMPCC_track_boundaries(matSpline,config_data,track_length,extended_distance,store_mex_data) %TO DO - ADD ELLIPSE CONSTRAINT, add cost term regarding the slip angle in order to reduce aggressiveness (AMZ), use the derivative of the control input, incorporate lift force(downforce), add cone related constraints, look into the warm start, check for inequalities that depend on consecutive timesteps, might be necessary to take the last ones, tire models need a closer look, add the jacobian (might speed up), experiment with different solvers to see which one performs better, decouple the throttle from the problem and send that command to a PID controller like pure pursuit did, fazer um sampling da centerline adaptativo a curvatura
...
%     distYellow_FORCES = ForcesInterpolation(matSpline.distanceYellowSpline);
%     distBlue_FORCES = ForcesInterpolation(matSpline.distanceBlueSpline);
...
%         x(i) = xSpline_FORCES(track_length*(i-1)/(SPLINE_POINTS*UPSAMPLE-1));
%         y(i) = ySpline_FORCES(track_length*(i-1)/(SPLINE_POINTS*UPSAMPLE-1));
%         distance_yellow(i) = distYellow_FORCES(track_length*(i-1)/(SPLINE_POINTS*UPSAMPLE-1));
%         distance_blue(i) = distBlue_FORCES(track_length*(i-1)/(SPLINE_POINTS*UPSAMPLE-1));
%     end
%     
% 
...
% 
%     figure
%     plot(distance_yellow,'Color',[0.9290 0.6940 0.1250])
%     hold on 
%     plot(distance_blue,'Color',[0 0.4470 0.7410])
...
%     delta_s_max = config_data.mpc_params.ineq_limits.delta_s_max; 
    ...
    % State Transition
    model.eq{1} = @(z,p) [z(1); z(2); z(5); progress_dynamics(z(5),z(8),z(9),z(10)); vehicle_dynamics(z(6:11),z(1:4),p)];
    model.E{1} = [zeros(model.neq(1),n_input+slack_variables) eye(model.neq(1))];

    % Simple Inequalities
    %             [d(t)  delta(t) d(t-1)   delta(t-1)  s(t)                              X     Y     Psi   vx    vy    r   ]
    model.lb{1} = [-inf, -inf,    -inf,    -inf,       0,                                -inf, -inf, -inf, -inf, -inf, -inf];
    model.ub{1} = [+inf, +inf,    +inf,    +inf,       track_length+extended_distance ,  +inf, +inf, +inf, +inf, +inf, +inf];
...
%     model.hu{1} = [+inf, +inf];
    ...
    for i=2:N-2
        % Problem dimensions
        model.nvar(i) = slack_variables + n_input + additional_variables + n_state;
        model.neq(i) =  additional_variables + n_state;
        model.nh(i) = 9-4-2; %e_CL constraints (front and rear) 4, input rate constraints 2, progress rate 1, friction ellipse 2
    
        % Objective function
        model.objective{i} = @objectiveMiddle;
    
        % State Transition
        model.eq{i} = @(z,p) [z(2); z(3); z(7); progress_dynamics(z(7),z(10),z(11),z(12)); vehicle_dynamics(z(8:13),z(2:5),p)];
        model.E{i} = [zeros(model.neq(i),n_input+slack_variables) eye(model.neq(i))];
    
        % Simple Inequalities
        %             [epsilon(t)   d(t)          delta(t)      d(t-1)   delta(t-1)   s(t-1)   s(t)                            X     Y     Psi   vx    vy    r   ]
        model.lb{i} = [-1,        throttle_min, steering_min, -inf,    -inf,        -inf,    0,                              -inf, -inf, -inf, 0   , -inf, -inf];
        model.ub{i} = [+1,        throttle_max, steering_max, +inf,    +inf,        +inf,    track_length+extended_distance, +inf, +inf, +inf, +inf, +inf, +inf];
             
        % Inequalities
        model.ineq{i} = @ineqM;
%         model.hl{i} = [-inf, 0, -inf, 0, -1, -1, -1, -inf, -inf];
%         model.hu{i} = [0, +inf, 0 , +inf, 1, 1, 1, 1, 1];
%         model.hl{i} = [-inf, -inf, -inf, -inf, -1, -1, -1, -inf, -inf];
%         model.hu{i} = [+inf, +inf, +inf , +inf, 1, 1, 1, +inf, +inf];
%         model.hl{i} = [-1, -1, -1, -inf, -inf];
%         model.hu{i} = [ 1, 1, 1, 1, 1];
        model.hl{i} = [-1, -1, -1];
        model.hu{i} = [ 1, 1, 1];
    end
    ...
    % Problem dimensions
    model.nvar(N-1) = slack_variables + n_input + additional_variables + n_state;
    model.neq(N-1) =  2 + n_state; %progress, progress(t-1), states
    model.nh(N-1) = 9-4-2; %e_CL constraint 4, input rate constraints 2, progress rate 1, friction ellipse 2
   
    % Objective function
    model.objective{N-1} = @objectiveMiddle;
   
    % State Transition
    model.eq{N-1} = @(z,p) [z(7); progress_dynamics(z(7),z(10),z(11),z(12)); vehicle_dynamics(z(8:13),z(2:5),p)];
    model.E{N-1} = [zeros(model.neq(N-1),slack_variables) eye(model.neq(N-1))];
   
    % Simple Inequalities
    %               [epsilon(t)   d(t)          delta(t)      d(t-1)   delta(t-1)   s(t-1)   s(t)                            X     Y     Psi   vx    vy    r   ]
    model.lb{N-1} = [-1,        throttle_min, steering_min, -inf,    -inf,        -inf,    0,                              -inf, -inf, -inf, 0   , -inf, -inf];
    model.ub{N-1} = [+1,        throttle_max, steering_max, +inf,    +inf,        +inf,    track_length+extended_distance, +inf, +inf, +inf, +inf, +inf, +inf];
   
    % Inequalities
    model.ineq{N-1} = @ineqM;
%     model.hl{N-1} = [-inf, 0, -inf, 0, -1, -1, -1, -inf, -inf];
%     model.hu{N-1} = [0, +inf, 0 , +inf, 1, 1, 1, 1, 1];
%     model.hl{N-1} = [-inf, -inf, -inf, -inf, -1, -1, -1, -inf, -inf];
%     model.hu{N-1} = [+inf, +inf, +inf , +inf, 1, 1, 1, 1, 1];
%     model.hl{N-1} = [-1, -1, -1, -inf, -inf];
%     model.hu{N-1} = [ 1, 1, 1, 1, 1];
    model.hl{N-1} = [-1, -1, -1];
    model.hu{N-1} = [ 1, 1, 1];
    ...
    % Problem dimensions
    model.nvar(N) = n_state + 2 + slack_variables; % states + progress + progress(t-1)
    model.nh(N) = 5-4; %e_CL constraint 4 + progress contsraint 1

    % Objective function
    model.objective{N} = @objectiveLast;

    % Simple Inequalities
    %             [epsilon s(t-1)   s(t)                             X    Y     Psi    vx   vy    r   ]
    model.lb{N} = [-1,     -inf,    0,                              -inf, -inf, -inf, 0   , -inf, -inf];
    model.ub{N} = [+1,     +inf,    track_length+extended_distance, +inf, +inf, +inf, +inf, +inf, +inf];

    % Inequalities
    model.ineq{N} = @ineqN;
%     model.hl{N} = [-inf, 0, -inf, 0, -1];
%     model.hu{N} = [0, +inf, 0 , +inf, 1];
%     model.hl{N} = [-inf, -inf, -inf, -inf, -1];
%     model.hu{N} = [+inf, +inf, +inf, +inf, 1];
    model.hl{N} = [-1];
    model.hu{N} = [ 1];
...
%     codeoptions.noVariableElimination = 1;
    ...
    codeoptions.SolverType = 'InteriorPoint'; % SQP or Interior Point
    if store_mex_data
...
% 
%     f = alpha_CL*e_CL^e_CL_exp + alpha_L*e_L^2 + beta_steer*(z(2)-z(4))^2 + beta_throttle*(z(1)-z(3))^2; 
    f = 0;
end
% Middle
...
%     progress_prev = z(6);
    ...
    [g_x, g_y, dg_x, dg_y] = splineTrack(progress);
    ...
    f = alpha_CL*e_CL^2 + beta_epsilon*epsilon^2 + q_r*(r*vel)^2 + beta_steer*(delta-delta_prev)^2 + beta_throttle*(d-d_prev)^2 + exp(q_v_max*(vel-v_max)) - lambda*vk*integrator_stepsize;
end
% End
...
%     e_L = -dg_x*(X - g_x) - dg_y*(Y - g_y);
    ...
    f = alpha_CL*e_CL^2 + beta_epsilon*epsilon^2 + q_r*(r*vel)^2 + exp(q_v_max*(vel-v_max)) - lambda*vk*integrator_stepsize;
end
...
function blended = vehicle_dynamics(x,u,p)
    ...
    lambda_ = min(max((velocity - lambda_blend_min)/(lambda_blend_max - lambda_blend_min), 0), 1);

    % discritized vehicle model
    dyn_bike = RK4(x,u,@dynamic_bicycle_model, integrator_stepsize,p);

    %Discritized Kinematic Vehicle Model
    kin_bike = RK4(x,u,@kinematic_bicycle_model, integrator_stepsize,p);
...
function dX = dynamic_bicycle_model(x,u,p)
    ...
    F_x_ax = F_f_ax*cos(u(4)) - Ff_tire_load*sin(u(4)) + F_r_ax - F_roll - F_drag;
    a_x = (F_x_ax)/m - x(5)*x(6);
...
function h = ineqI(z,p)    
    ...
    vy_ = z(10);
    r_ =  z(11);
    x = [X_ Y_ Psi_ vx_ vy_ r_];
    % Inputs
    throttle_ = z(1);
    steering_ = z(2);
    u = [throttle_ steering_];
    ...
    % Static + Downforce
    Fz_f = Ff_z + Ff_downforce;
    Fz_r = Fr_z + Fr_downforce;

    % Slip Angles and Lateral Forces
    alpha_f = u(2) - atan2(x(5) + x(6)*l_f, x(4));
    alpha_r = atan2((x(5) - x(6)*l_r), x(4));

    Fy_f = Ff_z*D*sin(C*atan(B*alpha_f));
    Fy_r = Fr_z*D*sin(C*atan(B*alpha_r));
    ...
    % Torque Requests
    Fx_f = 2*eta_motor*T_front*GR/r_wheel*u(1);
    Fx_r = 2*eta_motor*T_rear*GR/r_wheel*u(1);

    % blend the two models
    velocity = sqrt(x(4)^2+x(5)^2);
    lambda_ = min(max((velocity - lambda_blend_min)/(lambda_blend_max - lambda_blend_min), 0), 1);
    friction_ellipse = lambda_ == 1;

    h = [friction_ellipse*((Fx_f/(mu_x*Fz_f))^2 + (Fy_f/(mu_y*Fz_f))^2);   % front tire friction ellipse
         friction_ellipse*((Fx_r/(mu_x*Fz_r))^2 + (Fy_r/(mu_y*Fz_r))^2)];  % rear tire friction ellipse                           
end
...
function h = ineqM(z,p)
    ...
    throttle_prev_ = z(4);
    steering_prev_ = z(5);
    u_prev = [throttle_prev_ steering_prev_];
...
% 
%     % REAR
%     [gr_x, gr_y, dgr_x, dgr_y] = splineTrack(s+rear_proj);
...
% 
%     % Get Distances
%     [front_dist_blue,front_dist_yellow] = distanceBoundaries(s+front_proj+s_compensation_rear);
%     [rear_dist_blue,rear_dist_yellow] = distanceBoundaries(s+rear_proj+s_compensation_front);
    
    h = [
%         (e_F_CL)-(front_dist_blue-width/2-safety_margin);         % track front blue (left)
%          (e_F_CL)+(front_dist_yellow-width/2-safety_margin);       % track front yellow (right)
%          (e_R_CL)-(rear_dist_blue-width/2-safety_margin);          % track rear blue (left)
%          (e_R_CL)+(rear_dist_yellow-width/2-safety_margin);        % track rear yellow (right)
         (u(1)-u_prev(1))/delta_throttle_max;                              % rate throttle
         (u(2)-u_prev(2))/delta_steering_max;                              % rate steering
         (s-s_prev)/delta_s_max;                                           % rate progress
%          friction_ellipse*((Fx_f/(mu_x*Fz_f))^2 + (Fy_f/(mu_y*Fz_f))^2);   % front tire friction ellipse
%          friction_ellipse*((Fx_r/(mu_x*Fz_r))^2 + (Fy_r/(mu_y*Fz_r))^2)];  % rear tire friction ellipse
        ];
end
...
%     front_vector = [cos(x(3));sin(x(3))];
%     orthogonal_front_vector = [-sin(x(3));cos(x(3))];
%     rear_vector = [-cos(x(3));-sin(x(3))];
%     orthogonal_rear_vector = [-sin(x(3));cos(x(3))];
% 
%     fl_vector = front_vector*(l_f/(l_f + l_r))*length + orthogonal_front_vector*width/2;
%     fr_vector = front_vector*(l_f/(l_f + l_r))*length - orthogonal_front_vector*width/2;
%     rl_vector = rear_vector*(l_r/(l_f + l_r))*length + orthogonal_rear_vector*width/2;
%     rr_vector = rear_vector*(l_r/(l_f + l_r))*length - orthogonal_rear_vector*width/2;
% 
%     fl_proj = fl_vector'*dg;
%     fr_proj = fr_vector'*dg;
%     rl_proj = rl_vector'*dg;
%     rr_proj = rr_vector'*dg;
% 
%     [fl_dist_blue,fl_dist_yellow] = distanceBoundaries(s+fl_proj);
%     [fr_dist_blue,fr_dist_yellow] = distanceBoundaries(s+fr_proj);
%     [rl_dist_blue,rl_dist_yellow] = distanceBoundaries(s+rl_proj);
%     [rr_dist_blue,rr_dist_yellow] = distanceBoundaries(s+rr_proj);

    
%     h = [(e_F_CL-epsilon)-(front_dist_blue-width/2-safety_margin);         % track front blue (left)
%          (e_F_CL+epsilon)+(front_dist_yellow-width/2-safety_margin);       % track front yellow (right)
%          (e_R_CL-epsilon)-(rear_dist_blue-width/2-safety_margin);          % track rear blue (left)
%          (e_R_CL+epsilon)+(rear_dist_yellow-width/2-safety_margin);        % track rear yellow (right)
%          (s-s_prev)/delta_s_max];      