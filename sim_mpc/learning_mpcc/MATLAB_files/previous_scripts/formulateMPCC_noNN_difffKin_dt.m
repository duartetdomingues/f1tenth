...
% Soft Front track constraints
    ...
    %       51 - beta_psi
    %       51:end - NN_weights (1991 parameters in model 18)
...
function blended = vehicle_dynamics(x,u,p)
    ...
    kin_x = [x(1); x(2); x(3); velocity];
    kin_y = ForwardEuler(kin_x,u,@kinematic_bicycle_model, integrator_stepsize,p);
    Beta = atan(l_r*tan(u(2))/(l_f+l_r));
    kin_bike = [kin_y(1); kin_y(2); kin_y(3); kin_y(4)*cos(Beta); kin_y(4)*sin(Beta); kin_y(4)*cos(Beta)/(l_r+l_f)*tan(u(2))];
%     kin_bike(5) = kin_bike(4)*sin(Beta);
%     kin_bike(6) = kin_bike(4)*cos(Beta)/(l_r+l_f)*tan(u(2));
%     kin_bike(4) = kin_bike(4)*cos(Beta);
...
function out = NNmodel(x,p)
    in0 = x(1,:)';
    in1 = x(2,:)';

    ind = 51 + 1; % CHANGE THIS PARAMETER
    % FC1
