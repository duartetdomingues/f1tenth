function formulateMPCC_track_boundaries_reset(matSpline,config_data,track_length,extended_distance,store_mex_data) %TO DO - ADD ELLIPSE CONSTRAINT, add cost term regarding the slip angle in order to reduce aggressiveness (AMZ), use the derivative of the control input, incorporate lift force(downforce), add cone related constraints, look into the warm start, check for inequalities that depend on consecutive timesteps, might be necessary to take the last ones, tire models need a closer look, add the jacobian (might speed up), experiment with different solvers to see which one performs better, decouple the throttle from the problem and send that command to a PID controller like pure pursuit did
    ...
    model.ub{1} = [throttle_max, steering_max, +inf,    +inf,       track_length+extended_distance ,  +inf, +inf, +inf, +inf, +inf, +inf];
    ...
    for i=2:N-2
        ...
        model.nh(i) = 9-4; %e_CL constraint 4, input rate constraints 2, progress rate 1, friction ellipse 2
...
%         model.hu{i} = [0, +inf, 0 , +inf, 1, 1, 1, 1, 1];
    ...
    model.nh(N-1) = 9-4; %e_CL constraint 4, input rate constraints 2, progress rate 1, friction ellipse 2
...
function f = objectiveInitial(z,p)
    ...
    f = beta_steer*(z(2)-z(4))^2 + beta_throttle*(z(1)-z(3))^2; 
end
% Middle
function f = objectiveMiddle(z,p)
    ...
    f = q_r*(z(12)*vel)^2 + alpha_CL*e_CL^e_CL_exp + alpha_L*e_L^2 + beta_steer*(z(2)-z(4))^2 + beta_throttle*(z(1)-z(3))^2 + exp(q_v_max*(vel-v_max)) - lambda*vk*integrator_stepsize;
end
% End
function f = objectiveLast(z,p)
    ...
    f = q_r*(z(8)*vel)^2 + alpha_CL*e_CL^e_CL_exp + alpha_L*e_L^2 + exp(q_v_max*(vel-v_max)) - lambda*vk*integrator_stepsize;
end
