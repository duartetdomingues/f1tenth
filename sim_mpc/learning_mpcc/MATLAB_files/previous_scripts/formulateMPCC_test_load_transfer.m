function formulateMPCC_test_load_transfer(matSpline,centerline) %TO DO - ADD ELLIPSE CONSTRAINT, add cost term regarding the slip angle in order to reduce aggressiveness (AMZ), use the derivative of the control input, incorporate lift force(downforce), add cone related constraints, look into the warm start, check for inequalities that depend on consecutive timesteps, might be necessary to take the last ones, tire models need a closer look, add the jacobian (might speed up), experiment with different solvers to see which one performs better, decouple the throttle from the problem and send that command to a PID controller like pure pursuit did, a friction ellipse está ativa mesmo quando o dynamic não está em vigor
    global config_data;
    global track_length;
...
function dX = dynamic_bicycle_model(x,u,p)
    ...
    delta_m = h_cog/(l_r+l_f)*m*a_x/g;

    % Static + Downforce + Load Transfer
    Ff_z = Ff_z + Ff_downforce - delta_m;
    Fr_z = Fr_z + Fr_downforce + delta_m;
...
function h = ineqM(z,p)
    ...
    h = [dg_y*(z(7) - g_x) - dg_x*(z(8) - g_y);         % e_CL 
         ...
         (Fx_f/(mu_x*Fz_f))^2 + (Fy_f/(mu_y*Fz_f))^2;   % front tire friction ellipse
         (Fx_r/(mu_x*Fz_r))^2 + (Fy_r/(mu_y*Fz_r))^2;]; % rear tire friction ellipse
end
