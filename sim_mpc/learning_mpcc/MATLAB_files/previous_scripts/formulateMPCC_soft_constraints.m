function formulateMPCC_soft_constraints(matSpline,centerline) %TO DO - ADD ELLIPSE CONSTRAINT, add cost term regarding the slip angle in order to reduce aggressiveness (AMZ), use the derivative of the control input, incorporate lift force(downforce), add cone related constraints, look into the warm start, check for inequalities that depend on consecutive timesteps, might be necessary to take the last ones, tire models need a closer look, add the jacobian (might speed up), experiment with different solvers to see which one performs better, decouple the throttle from the problem and send that command to a PID controller like pure pursuit did
    ...
    % z =  [1 - d(t)
    %       2 - delta(t)
    %       3 - epsilon_Ff - Friction ellipse front
    %       4 - epsilon_Fr - Friction ellipse rear
    %       5 - d(t-1)
    %       6 - delta(t-1)
    %       7 - s(t)
    %       8 - X 
    ...
    % z =  [1 - d(t) 
    %       2 - delta(t)
    %       3 - epsilon_eCL - Lateral error
    %       4 - epsilon_s - progress
    %       5 - epsilon_Ff - Friction ellipse front
    %       6 - epsilon_Fr - Friction ellipse rear
    %       7 - d(t-1) 
    %       8 - delta(t-1)
    %       9 - s(t-1)
    %       10 - s(t)
    ...
    for i=2:N-2
        ...
        model.nh(i) = 6; %e_CL constraint, input rate constraints, progress rate, friction ellipse (front and rear)
    ...
    model.nh(N-1) = 6; %e_CL constraint, input rate constraints, progress rate, friction ellipse (front and rear)
    ...
    % z =  [1 - epsilon_eCL - Lateral error
    %       2 - epsilon_s - progress
    %       3 - epsilon_Ff - Friction ellipse front
    %       4 - epsilon_Fr - Friction ellipse rear
    %       5 - s(t-1)
    %       6 - s(t)
    ...
    model.nh(N) = 4; %e_CL constraint + progress contsraint, friction ellipse (front and rear)
...
%     model.objective{N} =@(z) 0;
    ...
    % Inequalities
    model.ineq{N} = @ineqN;
    model.hl{N} = [e_CL_min, delta_s_min, -inf, -inf];
    model.hu{N} = [e_CL_max, delta_s_max, inf, inf];
...
function f = objectiveMiddle(z,p)
    ...
    f = q_r*z(12)^2 + alpha_CL*e_CL^e_CL_exp + alpha_L*e_L^2 + beta_steer*(z(2)-z(4))^2 + beta_throttle*(z(1)-z(3))^2 + exp(q_v_max*(vel-v_max)) - lambda*vk*integrator_stepsize;
end
% End
function f = objectiveLast(z,p)
    ...
    f = q_r*z(8)^2 + alpha_CL*e_CL^e_CL_exp + alpha_L*e_L^2 + exp(q_v_max*(vel-v_max)) - lambda*vk*integrator_stepsize;
end
...
function dX = dynamic_bicycle_model(x,u,p)
    ...
    a_x = (F_f*cos(u(2)) + F_r - F_roll - F_drag)/m;
    delta_Fz = h_cog/(l_r+l_f)*m*a_x;

    % Static + Downforce + Load Transfer
    Ff_z = Ff_z + Ff_downforce - delta_Fz;
    Fr_z = Fr_z + Fr_downforce + delta_Fz;
