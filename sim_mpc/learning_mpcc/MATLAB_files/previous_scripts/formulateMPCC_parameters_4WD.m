...
%     delta_s_max = config_data.mpc_params.ineq_limits.delta_s_max; 
    ...
    %       12 - steering_min
    %       13 - steering_max
    %       14 - delta_throttle_min
    %       15 - delta_throttle_max
    %       16 - delta_steering_min
    %       17 - delta_steering_max
    %       18 - e_CL_min
    %       19 - e_CL_max
    %       20 - delta_s_min
    %       21 - delta_s_max
    ...
    model.nh(1) = 6; %input rate constraints, throttle, steering, progress, v_x
...
%     model.ub{1} = [throttle_max, steering_max, +inf,    +inf,       track_length+EXTEND_CENTERLINE ,  +inf, +inf, +inf, +inf, +inf, +inf];
    ...
    % Inequalities
    model.ineq{1} = @ineqI;
    model.hl{1} = [-1, -1, -1, -1, 0, 0];
    model.hu{1} = [1, 1, 1, 1, track_length+EXTEND_CENTERLINE, +inf];
    ...
    for i=2:N-2
        ...
        model.nh(i) = 8; %e_CL constraint, input rate constraints, progress rate, throttle, steering, progress, v_x
...
%         model.ub{i} = [throttle_max, steering_max, +inf,    +inf,        +inf,    track_length+EXTEND_CENTERLINE, +inf, +inf, +inf, +inf, +inf, +inf];
        ...
        % Inequalities
        model.ineq{i} = @ineqM;
        model.hl{i} = [-1, -1, -1, -1, -1, -1, 0, 0];
        model.hu{i} = [1, 1, 1, 1, 1, 1, track_length+EXTEND_CENTERLINE, +inf];
    end
    ...
    model.nh(N-1) = 8; %e_CL constraint, input rate constraints, progress rate, throttle, steering, progress, v_x
...
%     model.ub{N-1} = [throttle_max, steering_max, +inf,    +inf,        +inf,  track_length+EXTEND_CENTERLINE, +inf, +inf, +inf, +inf, +inf, +inf];
    ...
    model.hl{N-1} = [-1, -1, -1, -1, -1, -1, 0, 0];
    model.hu{N-1} = [1, 1, 1, 1, 1, 1, track_length+EXTEND_CENTERLINE, +inf];
    ...
    model.nh(N) = 4; %e_CL constraint + progress contsraint + progress + v_x
...
%     model.ub{N} = [+inf,    track_length+EXTEND_CENTERLINE, +inf, +inf, +inf, +inf, +inf, +inf];
    ...
    model.ineq{N} = @ineqN;
    model.hl{N} = [-1, -1, 0, 0];
    model.hu{N} = [1, 1, track_length+EXTEND_CENTERLINE, +inf];
...
function f = objectiveInitial(z,p)
    ...
    f = alpha_CL*e_CL^e_CL_exp + alpha_L*e_L^2;
end
% Middle
function f = objectiveMiddle(z,p)
    ...
    f = q_v_y*z(11)^2 + alpha_CL*e_CL^e_CL_exp + alpha_L*e_L^2 + beta_steer*(z(2)-z(4))^2 + exp(q_v_max*(sqrt(z(10)^2+z(11)^2)-v_max)) - lambda*vk*integrator_stepsize;
end
% End
function f = objectiveLast(z,p)
    ...
    f = q_v_y*z(7)^2 + alpha_CL*e_CL^e_CL_exp + alpha_L*e_L^2 + exp(q_v_max*(sqrt(z(6)^2+z(7)^2)-v_max)) - lambda*vk*integrator_stepsize;
end
...
%     car.m = p(24);
%     car.I_z = p(25);
%     car.T_max_front = p(26);
%     car.T_max_rear = p(27);
%     car.T_brake_front = p(28);
%     car.T_brake_rear = p(29);
%     car.GR = p(30);
%     car.eta_motor = p(31);
%     car.r_wheel = p(32);
%     car.g = p(33);
%     car.C_roll = p(34);
%     car.rho = p(35);
%     car.lambda_blend_min = p(36);
%     car.lambda_blend_max = p(37);
%     car.C_d = p(38);
%     car.C_l = p(39);
%     car.B = p(40);
%     car.C = p(41);
%     car.D = p(42);
%     car.downforce_front = p(43);
%     car.downforce_rear = p(44);
% 
%     % Discritized Dynamic Vehicle Model
%     dyn_bike = RK4(x,u,@dynamic_bicycle_model, integrator_stepsize);
% 
%     % Discritized Kinematic Vehicle Model
%     velocity = sqrt(x(4)^2+x(5)^2);
%     kin_states_t = [x(1) x(2) x(3) velocity]';
%     kin_states_t_1 = RK4(kin_states_t,u,@kinematic_bicycle_model, integrator_stepsize);
% %     kin_bike(6) = sqrt(x(4)^2+x(5)^2)/(car.l_r+car.l_f)*tan(u(2));
%     Beta = atan(car.l_r/(car.l_f+car.l_r)*tan(u(2)));
%     kin_bike_1 = kin_states_t_1(1);
%     kin_bike_2 = kin_states_t_1(2);
%     kin_bike_3 = kin_states_t_1(3);
%     kin_bike_4 = kin_states_t_1(4)*cos(Beta);
%     kin_bike_5 = kin_states_t_1(4)*sin(Beta);
%     kin_bike_6 = kin_bike_4/(car.l_r+car.l_f)*tan(u(2));
%     kin_bike = [kin_bike_1;kin_bike_2;kin_bike_3;kin_bike_4;kin_bike_5;kin_bike_6];
%     
%     % blend the two models
%     lambda_ = min(max((velocity - car.lambda_blend_min)/(car.lambda_blend_max - car.lambda_blend_min), 0), 1);
%     
...
function dX = dynamic_bicycle_model(x,u)
    ...
    Ff_z = car.m * (car.l_f/(car.l_f + car.l_r)) * car.g;
    Fr_z = car.m * (car.l_r/(car.l_f + car.l_r)) * car.g;

    F_downforce = 0.5 * car.rho *car.C_l * x(4)^2;

    Ff_downforce = F_downforce * car.downforce_front;
    Fr_downforce = F_downforce * car.downforce_rear;
    ...
    Ff_y = -Ff_z*car.D*sin(car.C*atan(car.B*alpha_f));
    Fr_y = -Fr_z*car.D*sin(car.C*atan(car.B*alpha_r));

    % Considerar a diferença de peso e mudar o D accordingly
    
    t_bool = (u(1) < 0);
    T_front = (~t_bool)*car.T_max_front + t_bool*(car.T_brake_front);
    T_rear = (~t_bool)*car.T_max_rear + t_bool*(car.T_brake_rear);

%     T_front = car.T_max_front;
%     T_rear = car.T_max_rear;
   
    F_f = 2*car.eta_motor*T_front*car.GR/car.r_wheel*u(1);
    F_r = 2*car.eta_motor*T_rear*car.GR/car.r_wheel*u(1);
    ...
    M_z = (F_f*sin(u(2))+Ff_y*cos_delta)*car.l_f - Fr_y*car.l_r;
...
%     F_roll = car.C_roll*car.m*car.g;
%     
%     F_drag = 1/2*car.rho*car.C_d*(x(4)*cos(Beta))^2;
%     
%     F_f = 2*car.eta_motor*T_front*car.GR/car.r_wheel*u(1);
%     F_r = 2*car.eta_motor*T_rear*car.GR/car.r_wheel*u(1);
% 
...
% % %           F_x/car.m + x(5)*x(6);...
% % %           F_y/car.m - x(4)*x(6);...
% % %           (F_y/car.m - x(4)*x(6))/car.m/car.l_r
% %           F_x/car.m;...
% %           F_y/car.m;...
% % %           F_y/car.m/car.l_r
% %           F_x/car.m/(car.l_f+car.l_r)*tan(u(2))];
% 
%     % State derivative function
%     cos_Phi = cos(x(3));
%     sin_Phi = sin(x(3));
...
%     dX = [x(4)*cos(x(3)+Beta);...
%           x(4)*sin(x(3)+Beta);...
%           x(4)*cos(Beta)/(car.l_f+car.l_r)*tan(u(2));...
%           F/car.m];
% end
...
function h = ineqI(z,p)  
    throttle_max = p(11);
    steering_max = p(13);
    delta_throttle_max = p(15);
    delta_steering_max = p(17);

    h = [(z(1)-z(3))/delta_throttle_max;    % rate throttle
         (z(2)-z(4))/delta_steering_max;    % rate steering
         z(1)/throttle_max;                 % throttle
         z(2)/steering_max;                 % steering
         z(5);                              % progress
         z(9)];                             % v_x
end
...
function [e_CL,e_L] = getErrors(x,y,s)
