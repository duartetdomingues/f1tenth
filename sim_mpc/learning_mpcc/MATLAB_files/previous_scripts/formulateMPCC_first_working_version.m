...
%     xSpline_FORCES = ForcesInterpolationFit(progress_list,centerline.X);
%     ySpline_FORCES = ForcesInterpolationFit(progress_list,centerline.Y);
%     dxSpline_FORCES = @(s) 1;
%     dySpline_FORCES = @(s) 0;
...
%     [g_x, g_y, dg_x, dg_y] = splineTrack(z(3));
    g_x = xSpline_FORCES(z(3));
    g_y = ySpline_FORCES(z(3));
    dg_x = dxSpline_FORCES(z(3));
    dg_y = dySpline_FORCES(z(3));
    ...
    f = alpha_CL*e_CL^2 + alpha_L*e_L^2;
end
% Middle
...
%     [g_x, g_y, dg_x, dg_y] = splineTrack(z(6));
    g_x = xSpline_FORCES(z(6));
    g_y = ySpline_FORCES(z(6));
    dg_x = dxSpline_FORCES(z(6));
    dg_y = dySpline_FORCES(z(6));
    ...
    f = q_v_y*z(11)^2 + alpha_CL*e_CL^2 + alpha_L*e_L^2 + beta_steer*(z(1)-z(3))^2 + exp(q_v_max*(z(10)-v_max));
end
% End
...
%     [g_x, g_y, dg_x, dg_y] = splineTrack(z(2));
    g_x = xSpline_FORCES(z(2));
    g_y = ySpline_FORCES(z(2));
    dg_x = dxSpline_FORCES(z(2));
    dg_y = dySpline_FORCES(z(2));
    ...
    f = q_v_y*z(7)^2 + alpha_CL*e_CL^2 + alpha_L*e_L^2 + exp(q_v_max*(z(6)-v_max)) + (-lambda*z(2));
end
...
function progress = progress_dynamics(s,yaw,vx,vy)
    ...
    dg_x = dxSpline_FORCES(s);
    dg_y = dySpline_FORCES(s);
