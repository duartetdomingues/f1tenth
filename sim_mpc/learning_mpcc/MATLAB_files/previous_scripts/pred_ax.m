...
for i=1:length(throttle)
    ...
    F_f_ax = 2*eta_motor*T_front*GR/r_wheel*throttle_prev(i);
    F_r_ax = 2*eta_motor*T_rear*GR/r_wheel*throttle_prev(i);
    Ff_tire_load = (Ff_z_static+Ff_downforce)*D*sin(C*atan(B*alpha_f));
    F_x_ax = F_f_ax*steering_prev(i) - Ff_tire_load*sin(steering_prev(i)) + F_r_ax - F_roll - F_drag;
    a_x(i) = (F_x_ax)/m - vy(i)*r(i);
end

plot(a_x);
plot(zeros(1,length(a_x)));