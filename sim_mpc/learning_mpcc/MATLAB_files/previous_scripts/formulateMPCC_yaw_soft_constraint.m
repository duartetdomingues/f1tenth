...
% Soft Front track constraints
    ...
    for i=2:N-2
        % Problem dimensions
        model.nvar(i) = prev_variables + n_input + additional_variables + n_state + slack_variables;
        ...
        % State Transition
        model.eq{i} = @(z,p) [z(2); z(3); z(13); z(14); z(15); progress_dynamics(z(9),z(12),z(13),z(14)); vehicle_dynamics(z(10:15),z(2:5),[p; z(6:8)])];
        ...
        model.lb{i} = [-inf,    throttle_min, steering_min, -inf,    -inf,       -inf,    -inf,    -inf,   0,                                -inf, -inf, -inf, 0   , -inf, -inf];
        model.ub{i} = [+inf,    throttle_max, steering_max, +inf,    +inf,       +inf,    +inf,    +inf,   track_length+extended_distance ,  +inf, +inf, +inf, +inf, +inf, +inf];
    ...
    % Problem dimensions
    model.nvar(N-1) = prev_variables + n_input + additional_variables + n_state + slack_variables;
    ...
    % State Transition
    model.eq{N-1} = @(z,p) [progress_dynamics(z(9),z(12),z(13),z(14)); vehicle_dynamics([z(10:15)],z(2:5),[p; z(6:8)])];
    model.E{N-1} = [eye(model.neq(N-1))];
   
    % Simple Inequalities
    %               [epsilon  d(t)          delta(t)      d(t-1)   delta(t-1)  vx(t-1)  vy(t-1)  r(t-1)  s(t)                              X     Y     Psi   vx    vy    r   ]
    model.lb{N-1} = [-inf,    throttle_min, steering_min, -inf,    -inf,       -inf,    -inf,    -inf,   0,                                -inf, -inf, -inf, 0   , -inf, -inf];
    model.ub{N-1} = [+inf,    throttle_max, steering_max, +inf,    +inf,       +inf,    +inf,    +inf,   track_length+extended_distance ,  +inf, +inf, +inf, +inf, +inf, +inf];
...
function f = objectiveMiddle(z,p)
    ...
    f = beta_epsilon*epsilon^2 + q_r*(r*vel)^2 + alpha_CL*e_CL^2 + alpha_L*e_L^2 + 1/integrator_stepsize*beta_steer*(steering-steering_prev)^2 + 1/integrator_stepsize*beta_throttle*(throttle-throttle_prev)^2 + exp(q_v_max*(vel-v_max)) - lambda*vk*integrator_stepsize;
end
% End
