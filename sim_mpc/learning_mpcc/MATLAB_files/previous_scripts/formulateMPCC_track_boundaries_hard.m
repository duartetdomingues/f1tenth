...
%     model.hu{1} = [+inf, +inf];
    ...
    for i=2:N-2
        ...
        model.nh(i) = 0; %e_CL constraints (front and rear) 4, input rate constraints 2, progress rate 1, friction ellipse 2
    
        % Objective function
        model.objective{i} = @objectiveMiddle;
    
        % State Transition
        model.eq{i} = @(z,p) [z(1); z(2); z(6); progress_dynamics(z(6),z(9),z(10),z(11)); vehicle_dynamics(z(7:12),z(1:4),p)];
        ...
        model.ub{i} = [throttle_max, steering_max, +inf,    +inf,        +inf,    track_length+extended_distance, +inf, +inf, +inf, +inf, +inf, +inf];
...
%         model.hu{i} = [ 1, 1, 1];
    ...
    model.nh(N-1) = 0; %e_CL constraint 4, input rate constraints 2, progress rate 1, friction ellipse 2
   
    % Objective function
    model.objective{N-1} = @objectiveMiddle;
   
    % State Transition
    model.eq{N-1} = @(z,p) [z(6); progress_dynamics(z(6),z(9),z(10),z(11)); vehicle_dynamics(z(7:12),z(1:4),p)];
    model.E{N-1} = eye(model.neq(N-1));
   
    % Simple Inequalities
    %               [d(t)          delta(t)      d(t-1)   delta(t-1)   s(t-1)   s(t)                            X     Y     Psi   vx    vy    r   ]
    model.lb{N-1} = [throttle_min, steering_min, -inf,    -inf,        -inf,    0,                              -inf, -inf, -inf, 0   , -inf, -inf];
    model.ub{N-1} = [throttle_max, steering_max, +inf,    +inf,        +inf,    track_length+extended_distance, +inf, +inf, +inf, +inf, +inf, +inf];
...
%     model.hu{N-1} = [ 1, 1, 1];
    ...
    model.nh(N) = 0; %e_CL constraint 4 + progress contsraint 1
    ...
    model.ub{N} = [+inf,    track_length+extended_distance, +inf, +inf, +inf, +inf, +inf, +inf];
...
%     progress_prev = z(5);
    ...
    f = alpha_CL*e_CL^2 + q_r*(r*vel)^2 + beta_steer*(delta-delta_prev)^2 + beta_throttle*(d-d_prev)^2 + exp(q_v_max*(vel-v_max)) - lambda*vk*integrator_stepsize;
end
% End
...
%     e_L = -dg_x*(X - g_x) - dg_y*(Y - g_y);
    ...
    f = alpha_CL*e_CL^2 + q_r*(r*vel)^2 + exp(q_v_max*(vel-v_max)) - lambda*vk*integrator_stepsize;
end
