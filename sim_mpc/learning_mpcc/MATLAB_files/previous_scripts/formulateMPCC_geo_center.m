...
function f = objectiveMiddle(z,p)
    ...
    X_geo = X + cos(yaw);
    Y_geo = Y + sin(yaw);
    e_CL = dg_y*(X_geo - g_x) - dg_x*(Y_geo - g_y);
    e_L = -dg_x*(X_geo - g_x) - dg_y*(Y_geo - g_y);

    vel = sqrt(vx^2+vy^2);

    f = q_r*(r*vel)^2 + alpha_CL*e_CL^2 + alpha_L*e_L^2 + 1/integrator_stepsize*beta_steer*(steering-steering_prev)^2 + 1/integrator_stepsize*beta_throttle*(throttle-throttle_prev)^2 + exp(q_v_max*(vel-v_max)) - lambda*vk*integrator_stepsize;
end
% End
