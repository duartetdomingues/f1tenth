...
% Yellow
yellow_arc_length = 0;
% yellow_progress = zeros(2*length(track_data.X_y),1);
yellow_progress = zeros(length(track_data.X_y)+1,1);
for i=1:length(track_data.X_y)-1
    yellow_arc_length = yellow_arc_length + sqrt((track_data.X_y(i+1)-track_data.X_y(i))^2+(track_data.Y_y(i+1)-track_data.Y_y(i))^2);
    yellow_progress(i+1) = yellow_arc_length;
end
yellow_arc_length = yellow_arc_length + sqrt((track_data.X_y(1)-track_data.X_y(end))^2+(track_data.Y_y(1)-track_data.Y_y(end))^2);
yellow_progress(length(track_data.X_y)+1) = yellow_arc_length;
% for i=1:length(track_data.X_y)-1
%     yellow_arc_length = yellow_arc_length + sqrt((track_data.X_y(i+1)-track_data.X_y(i))^2+(track_data.Y_y(i+1)-track_data.Y_y(i))^2);
%     yellow_progress(length(track_data.X_y)+i+1) = yellow_arc_length;
% end

% matSpline.xYellowCones = spline(yellow_progress,[track_data.X_y; track_data.X_y]);
% matSpline.yYellowCones = spline(yellow_progress,[track_data.Y_y; track_data.Y_y]);
matSpline.xYellowCones = spline(yellow_progress,[track_data.X_y; track_data.X_y(1)]);
matSpline.yYellowCones = spline(yellow_progress,[track_data.Y_y; track_data.Y_y(1)]);

for i=1:1001
    yellow_point_x(i) = fnval(matSpline.xYellowCones,yellow_arc_length/3*(i-1)/1000);
    yellow_point_y(i) = fnval(matSpline.yYellowCones,yellow_arc_length/3*(i-1)/1000);
end

% % Blue
% blue_arc_length = 0;
% blue_progress = zeros(length(track_data.X_b),1);
% for i=1:length(track_data.X_b)-1
%     blue_arc_length = blue_arc_length + sqrt((track_data.X_b(i+1)-track_data.X_b(i))^2+(track_data.Y_b(i+1)-track_data.Y_b(i))^2);
%     blue_progress(i+1) = blue_arc_length;
% end
% yellow_arc_length = yellow_arc_length + sqrt((track_data.X_y(1)-track_data.X_y(end))^2+(track_data.Y_y(1)-track_data.Y_y(end))^2);
% yellow_progress(length(track_data.X_y)+1) = yellow_arc_length;
% for i=1:length(track_data.X_y)-1
...
% end
% 
% matSpline.xBlueCones = spline(blue_progress,track_data.X_b);
% matSpline.yBlueCones = spline(blue_progress,track_data.Y_b);
% 
% for i=1:1001
%     blue_point_x(i) = fnval(matSpline.xBlueCones,blue_arc_length*(i-1)/1000);
%     blue_point_y(i) = fnval(matSpline.yBlueCones,blue_arc_length*(i-1)/1000);
% end
