function formulateMPCC_accel(track_data) %TO DO - ADD ELLIPSE CONSTRAINT, look into the interpolation for the track splines, add cost term regarding the slip angle in order to reduce aggressiveness (AMZ), use the derivative of the control input, incorporate lift force(downforce), add cone related constraints, look into the warm start, check for inequalities that depend on consecutive timesteps, might be necessary to take the last ones, tire models need a closer look, add the jacobian (might speed up), experiment with different solvers to see which one performs better
    ...
    new_centerline.X = zeros(1,floor(length(centerline.X)/DOWNSAMPLE_RATE)+1);
    new_centerline.Y = zeros(1,floor(length(centerline.X)/DOWNSAMPLE_RATE)+1);
    new_progress_list = zeros(1,floor(length(centerline.X)/DOWNSAMPLE_RATE)+1);
    
    if DOWNSAMPLE_FLAG
        for i=1:floor(length(centerline.X)/DOWNSAMPLE_RATE)
            new_centerline.X(i) = centerline.X(i*DOWNSAMPLE_RATE-(DOWNSAMPLE_RATE-1));
            new_centerline.Y(i) = centerline.Y(i*DOWNSAMPLE_RATE-(DOWNSAMPLE_RATE-1));
            new_progress_list(i) = progress_list(i*DOWNSAMPLE_RATE-(DOWNSAMPLE_RATE-1));
        end
        new_centerline.X(end) = centerline.X(end);
        new_centerline.Y(end) = centerline.Y(end);
        new_progress_list(end) = progress_list(end);
    else
        new_centerline.X = centerline.X;
        new_centerline.Y = centerline.Y;
        new_progress_list = progress_list;
    end

    % Matlab Splines and Derivatives
    xSpline = spline(new_progress_list,new_centerline.X);
    ySpline = spline(new_progress_list,new_centerline.Y);
    ...
    % spline derivative
    x = xSpline_FORCES(linspace(0,track_length,25));
    y = ySpline_FORCES(linspace(0,track_length,25));
    P1 = [x' y'];
    dx = dxSpline_FORCES(linspace(0,track_length,25));
    dy = dySpline_FORCES(linspace(0,track_length,25));
    P2 = [(x+2*dx)' (y+2*dy)'];
    ...
    %% First timestep - MAYBE ADD XINIT IN THE INPUTS AS WELL , ADD PARAMETERS
    ...
    % Problem dimensions
    model.nvar(1) = n_input + 1 + n_state + 1; % input + progress + n_state + one additional variable for FORCESPRO being choninhas
    model.xinitidx = 3:9;
    model.neq(1) =  additional_variables + n_state;
    model.nh(1) = 1; %only the error e_CL constraint

    % Objective function
    model.objective{1} = @objectiveInitial;

    % State Transition
    model.eq{1} = @(z) [z(1); z(2); z(3); progress_dynamics(z(3),z(6),z(7),z(8)); vehicle_dynamics(z(4:9),z(1:2))];
    model.E{1} = [zeros(model.neq(1),n_input) eye(model.neq(1))];

    % Simple Inequalities
    %             [d(t)          delta(t)      s(t)  X    Y     Psi    vx    vy    r     choninhas];
    model.lb{1} = [throttle_min, steering_min, -inf, -inf, -inf, -inf, -inf, -inf, -inf, -1       ];
    model.ub{1} = [throttle_max, steering_max, +inf, +inf, +inf, +inf, +inf, +inf, +inf, +1       ];
    ...
    for i=2:N-2
        ...
        model.lb{i} = [throttle_min, steering_min, -inf,    -inf,        -inf,    0,            -inf, -inf, -inf, -inf, -inf, -inf];
        model.ub{i} = [throttle_max, steering_max, +inf,    +inf,        +inf,    track_length, +inf, +inf, +inf, +inf, +inf, +inf];
    ...
    % Simple Inequalities
    %             [d(t)          delta(t)      d(t-1)   delta(t-1)   s(t-1)   s(t)            X     Y     Psi   vx    vy    r   ]
    model.lb{N-1} = [throttle_min, steering_min, -inf,    -inf,        -inf,    0,            -inf, -inf, -inf, -inf, -inf, -inf];
    model.ub{N-1} = [throttle_max, steering_max, +inf,    +inf,        +inf,    track_length, +inf, +inf, +inf, +inf, +inf, +inf];
    ...
    model.lb{N} = [-inf,    0,            -inf, -inf, -inf, -inf, -inf, -inf];
    model.ub{N} = [+inf,    track_length, +inf, +inf, +inf, +inf, +inf, +inf];
...
function f = objectiveInitial(z,p)
    ...
    [g_x, g_y, dg_x, dg_y] = splineTrack(z(3));

    temp = sqrt(dg_x^2 + dg_y^2);
    dg_x = dg_x/temp;
    dg_y = dg_y/temp;
    
    e_CL = dg_y*(z(4) - g_x) - dg_x*(z(5) - g_y);
    e_L = -dg_x*(z(4) - g_x) - dg_y*(z(5) - g_y);
...
function f = objectiveMiddle(z,p)
    ...
    f = q_v_y*z(11)^2 + alpha_CL*e_CL^e_CL_exp + alpha_L*e_L^2 + beta_steer*(z(1)-z(3))^2 + exp(q_v_max*(z(10)-v_max));
end
% End
function f = objectiveLast(z,p)
    ...
    f = q_v_y*z(7)^2 + alpha_CL*e_CL^e_CL_exp + alpha_L*e_L^2 + exp(q_v_max*(z(6)-v_max)) + (-lambda*z(2));
end
...
function h = ineqI(z)
    ...
    h = dg_y*(z(4) - g_x) - dg_x*(z(5) - g_y); % e_CL
end
