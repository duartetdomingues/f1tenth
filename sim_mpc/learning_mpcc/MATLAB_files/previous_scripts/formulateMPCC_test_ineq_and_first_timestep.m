...
%     hold off
    ...
    for i=2:N-2
        ...
        model.nh(i) = 6; %e_CL constraint, input rate constraints, progress rate, friction ellipse
        ...
        % Inequalities
        model.ineq{i} = @ineqM;
        model.hl{i} = [e_CL_min, -1, -1, -1, -inf, -inf];
        model.hu{i} = [e_CL_max, 1, 1, 1, 1, 1];
    end
    ...
    model.nh(N-1) = 6; %e_CL constraint, input rate constraints, progress rate, friction ellipse
    ...
    model.hl{N-1} = [e_CL_min, -1, -1, -1, -inf, -inf];
    model.hu{N-1} = [e_CL_max, 1, 1, 1, 1, 1];
    ...
    % Inequalities
    model.ineq{N} = @ineqN;
    model.hl{N} = [e_CL_min, -1];
    model.hu{N} = [e_CL_max, 1];
