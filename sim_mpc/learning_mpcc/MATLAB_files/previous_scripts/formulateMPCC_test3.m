...
%     car.D = config_data.tyre_params.D;
    ...
    %% First timestep
    % É NECESSÁRIO MUDAR AS MENSAGENS E ISSO TUDO NO CPP
    % z =  [1 - d(t)
    %       2 - delta(t)
    %       3 - s(t)
    %       4 - X 
    ...
    % Problem dimensions
    model.nvar(1) = n_input + 1 + n_state + 1; % input + progress + states + dummy
    model.xinitidx = 1:9;
    ...
    % State Transition
    model.eq{1} = @(z,p) [z(1); z(2); z(3); progress_dynamics(z(3),z(6),z(7),z(8)); vehicle_dynamics(z(4:9),z(1:2),p)];
    model.E{1} = [zeros(model.neq(1),n_input) eye(model.neq(1))];

    % Simple Inequalities
    %             [d(t)          delta(t)       s(t)                              X     Y     Psi   vx    vy    r     dummy]
    model.lb{1} = [throttle_min, steering_min,  0,                                -inf, -inf, -inf, 0   , -inf, -inf, 0];
    model.ub{1} = [throttle_max, steering_max,  track_length+EXTEND_CENTERLINE ,  +inf, +inf, +inf, +inf, +inf, +inf, 1];
