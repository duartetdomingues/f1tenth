...
% Soft Front track constraints
    ...
    additional_variables = 3; % d(t-1), delta(t-1), s(t)
    ...
    % Problem dimensions
    model.nvar(1) = n_input + additional_variables + n_state; % input + all additional states + states
    model.xinitidx = 1:11;
    ...
    % State Transition
    model.eq{1} = @(z,p) [z(1); z(2); progress_dynamics(z(5),z(8),z(9),z(10)); vehicle_dynamics(z(6:11),z(1:4),p)];
    ...
    for i=2:N-2
        ...
        % State Transition
        model.eq{i} = @(z,p) [z(1); z(2); progress_dynamics(z(5),z(8),z(9),z(10)); vehicle_dynamics(z(6:11),z(1:4),p)];
        model.E{i} = [zeros(model.neq(i),n_input) eye(model.neq(i))];
    
        % Simple Inequalities
        %             [d(t)          delta(t)      d(t-1)   delta(t-1)   s(t)                            X     Y     Psi   vx    vy    r   ]
        model.lb{i} = [throttle_min, steering_min, -inf,    -inf,        0,                              -inf, -inf, -inf, 0   , -inf, -inf];
        model.ub{i} = [throttle_max, steering_max, +inf,    +inf,        track_length+extended_distance, +inf, +inf, +inf, +inf, +inf, +inf];
    ...
    model.neq(N-1) =  1 + n_state; %progress, states
    ...
    % State Transition
    model.eq{N-1} = @(z,p) [progress_dynamics(z(5),z(8),z(9),z(10)); vehicle_dynamics(z(6:11),z(1:4),p)];
    model.E{N-1} = [eye(model.neq(N-1))];
   
    % Simple Inequalities
    %               [d(t)          delta(t)      d(t-1)   delta(t-1)  s(t)                          X     Y     Psi   vx    vy    r   ]
    model.lb{N-1} = [throttle_min, steering_min, -inf,    -inf,       0,                              -inf, -inf, -inf, 0   , -inf, -inf];
    model.ub{N-1} = [throttle_max, steering_max, +inf,    +inf,       track_length+extended_distance, +inf, +inf, +inf, +inf, +inf, +inf];
    ...
    % Problem dimensions
    model.nvar(N) = n_state + 1; % states + progress
    model.nh(N) = 0;
