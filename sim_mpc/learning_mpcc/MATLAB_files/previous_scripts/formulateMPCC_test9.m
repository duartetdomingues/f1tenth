...
% Hard track constraints
    ...
    model.nh(1) = 4; %input rate constraints + friction ellipse
    ...
    for i=2:N-2
        ...
        model.nh(i) = 9; %e_CL constraint 4, input rate constraints 2, progress rate 1, friction ellipse 2
    ...
    model.nh(N-1) = 9; %e_CL constraint, input rate constraints, progress rate, friction ellipse
    ...
    model.nh(N) = 5; %e_CL constraint 3 + progress contsraint 1
...
function h = ineqI(z,p)    
    ...
    h = [(z(1)-z(3))/delta_throttle_max;                             % rate throttle
         (z(2)-z(4))/delta_steering_max;                             % rate steering
...
function h = ineqM(z,p)
    ...
    h = [
         ...
         (z(1)-z(3))/delta_throttle_max;                             % rate throttle
         (z(2)-z(4))/delta_steering_max;                             % rate steering
         (z(6)-z(5))/delta_s_max;                            % rate progress
         friction_ellipse*((Fx_f/(mu_x*Fz_f))^2 + (Fy_f/(mu_y*Fz_f))^2);   % front tire friction ellipse
         friction_ellipse*((Fx_r/(mu_x*Fz_r))^2 + (Fy_r/(mu_y*Fz_r))^2)];  % rear tire friction ellipse
...
function h = ineqN(z,p)
    ...
    % Get Distances
    [front_dist_blue,front_dist_yellow] = distanceBoundaries(s+front_proj+s_compensation_front);
    [rear_dist_blue,rear_dist_yellow] = distanceBoundaries(s+rear_proj+s_compensation_rear);
    ...
    h = [
         ...
         (e_R_CL)-(rear_dist_blue-width/2-safety_margin);          % track rear blue (left)
         (e_R_CL)+(rear_dist_yellow-width/2-safety_margin);        % track rear yellow (right)
         (z(2)-z(1))/delta_s_max];                            % rate progress
end
