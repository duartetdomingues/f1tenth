...
function dX = dynamic_bicycle_model(x,u)
    ...
    F_f = 2*car.eta_motor*T_front*car.GR/car.r_wheel*u(1) - F_roll/2;
    F_r = 2*car.eta_motor*T_rear*car.GR/car.r_wheel*u(1) - F_roll/2;
    
    F_drag = 1/2*car.rho*car.C_d*x(4)^2;
    
    F_x = F_f*cos(u(2)) + F_r - F_drag - Ff_y*sin(u(2));
    F_y = Ff_y*cos(u(2)) + F_f*sin(u(2)) + Fr_y;
...
%           F_y/car.m-x(4)*x(6);...
          F_x/car.m;...
          F_y/car.m;...
          1/car.I_z*((Ff_y*cos(u(2)) + F_f*sin(u(2)))*car.l_f + Fr_y*car.l_r)];
end
