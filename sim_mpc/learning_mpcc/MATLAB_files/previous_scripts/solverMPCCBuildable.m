...
classdef solverMPCCBuildable < coder.ExternalDependency
    ...
    methods (Static)
        ...
        function [output,exitflag,info] = forcesInitOutputsMatlab()
            ...
            outputs_z = coder.nullcopy(zeros(544, 1));
            outputs_zN_1 = coder.nullcopy(zeros(14, 1));
            outputs_zN = coder.nullcopy(zeros(7, 1));
        ...
        function [output,exitflag,info] = forcesCallWithParams(params)
            [output,exitflag,info] = solverMPCCBuildable.forcesCall(params.xinit, params.x0, params.all_parameters, params.solver_timeout, params.num_of_threads);
        end

        function [output,exitflag,info] = forcesCall(xinit, x0, all_parameters, solver_timeout, num_of_threads)
    ...
    methods (Static, Access = private)
        function [output,exitflag,info] = forcesInitOutputsC()
            ...
            outputs_z = coder.nullcopy(double(zeros(544, 1)));
            outputs_zN_1 = coder.nullcopy(double(zeros(14, 1)));
            outputs_zN = coder.nullcopy(double(zeros(7, 1)));
            output = struct('z', outputs_z,...
                            'zN_1', outputs_zN_1,...
                            'zN', outputs_zN);
