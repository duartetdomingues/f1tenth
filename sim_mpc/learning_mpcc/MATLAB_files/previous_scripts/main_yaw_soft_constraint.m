...
SPLINE_POINTS = 55;
EXTENDED_POINTS = 10; % Zero is currently not supported
...
%     formulateMPCC_T1(matSpline, config_data, track_length, extended_distance,STORE_MEX_DATA);
%     formulateMPCC_T1_decode_encode(matSpline, config_data, track_length, extended_distance,STORE_MEX_DATA);
%     formulateMPCC_T1_decode_encode_kin(matSpline, config_data, track_length, extended_distance,STORE_MEX_DATA);
%     formulateMPCC_T1_decode_encode_relu(matSpline, config_data, track_length, extended_distance,STORE_MEX_DATA);
%     formulateMPCC_T1_decode_encode_params(matSpline, config_data, track_length, extended_distance,STORE_MEX_DATA);
%     formulateMPCC_geo_center(matSpline, config_data, track_length, extended_distance,STORE_MEX_DATA); 
%     formulateMPCC_kamm_circle(matSpline, config_data, track_length, extended_distance,STORE_MEX_DATA); 
%     formulateMPCC_yaw_constraint(matSpline, config_data, track_length, extended_distance,STORE_MEX_DATA); 
    formulateMPCC_yaw_soft_constraint(matSpline, config_data, track_length, extended_distance,STORE_MEX_DATA); 
end
...
%     jacDynamicsN = zeros(8,config_data.mpc_params.objective.z,duration);
    ...
    allzI = zeros(config_data.mpc_params.objective.z-1,duration);

    % Set initial guess to start solver from (here, middle of upper and lower bound)
%     x0iI=[0.01,0.01,0.01,0.01,0.01,0.01,0.01,0.01,0.01,0.01,0.01]';
%     x0iM=[0.01,0.01,0.01,0.01,0.01,0.01,0.01,0.01,0.01,0.01,0.01,0.01,0.01]';
%     x0iZ=[0.01,0.01,0.01,0.01,0.01,0.01,0.01,0.01,0.01]';
    x0iI=[0.01,0.01,0.01,0.01,0.01,0.01,0.01,0.01,0.01,0.01,0.01,0.01,0.01,0.01]';
    x0iM=[0.01,0.01,0.01,0.01,0.01,0.01,0.01,0.01,0.01,0.01,0.01,0.01,0.01,0.01,0.01]';
...
%         problem.xinit = [output.z(2),output.z(3),output.zI(1),output.zI(2),output.z(7),output.z(8),output.z(9),output.z(10),output.z(11),output.z(12),output.z(13)]';
        problem.xinit = [output.z(2),output.z(3),output.zI(1),output.zI(2),output.zI(12),output.zI(13),output.zI(14),output.z(9),output.z(10),output.z(11),output.z(12),output.z(13),output.z(14),output.z(15)]';
        i = i + 1;
    end
...
%     plot(xaxis_states, ones(1,duration)*config_data.mpc_params.ineq_limits.steering_max);
    ...
    title("v_y");
    nexttile
    velocity_idxs = states(4,:) > 5.0;
    velocity = states(4,velocity_idxs);
    diff = length(states(4,:))-length(velocity);
    yaw_rate_limit = 0.85*config_data.tyre_params.mu_x*config_data.model_params.g./velocity;
    yaw_rate_limit = [ones(1,diff)*yaw_rate_limit(1) yaw_rate_limit];
    ...
    hold on
    yline(config_data.mpc_params.ineq_limits.delta_throttle_min * config_data.controller_freq);
    yline(config_data.mpc_params.ineq_limits.delta_throttle_max * config_data.controller_freq);
...
%     plot(xaxis_states, ones(1,duration)*config_data.mpc_params.ineq_limits.delta_throttle_max);
    ...
    hold on
    yline(config_data.mpc_params.ineq_limits.delta_steering_min * config_data.controller_freq);
    yline(config_data.mpc_params.ineq_limits.delta_steering_max * config_data.controller_freq);
