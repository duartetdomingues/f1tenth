...
% Soft Front track constraints
    ...
    % Problem dimensions
    model.nvar(1) = n_input + prev_variables + additional_variables + n_state; % input + all additional states + states
    model.xinitidx = 1:14;
    model.neq(1) =  prev_variables + additional_variables + n_state;
    model.nh(1) = 2; % input rate constraints 2

    % Objective function
    model.objective{1} = @objectiveInitial;

    % State Transition
    model.eq{1} = @(z,p) [z(1); z(2); z(12); z(13); z(14); progress_dynamics(z(8),z(11),z(12),z(13)); vehicle_dynamics([z(9:14)],z(1:4),[p; z(5:7)])];
    model.E{1} = [zeros(model.neq(1),n_input) eye(model.neq(1))];

    % Simple Inequalities
    %             [d(t)          delta(t)      d(t-1)   delta(t-1)  vx(t-1)  vy(t-1)  r(t-1)  s(t)                              X     Y     Psi   vx    vy    r   ]
    model.lb{1} = [throttle_min, steering_min, -inf,    -inf,       -inf,    -inf,    -inf,   0,                                -inf, -inf, -inf, 0   , -inf, -inf];
    model.ub{1} = [throttle_max, steering_max, +inf,    +inf,       +inf,    +inf,    +inf,   track_length+extended_distance ,  +inf, +inf, +inf, +inf, +inf, +inf];
    ...
    for i=2:N-2
        % Problem dimensions
        model.nvar(i) = prev_variables + n_input + additional_variables + n_state;
        model.neq(i) =  prev_variables + additional_variables + n_state;
        model.nh(i) = 3; %input rate constraints 2
    
        % Objective function
        model.objective{i} = @objectiveMiddle;
    
        % State Transition
        model.eq{i} = @(z,p) [z(1); z(2); z(12); z(13); z(14); progress_dynamics(z(8),z(11),z(12),z(13)); vehicle_dynamics(z(9:14),z(1:4),[p; z(5:7)])];
        model.E{i} = [zeros(model.neq(i),n_input) eye(model.neq(i))];
    
        % Simple Inequalities
        %             [d(t)          delta(t)      d(t-1)   delta(t-1)  vx(t-1)  vy(t-1)  r(t-1)  s(t)                              X     Y     Psi   vx    vy    r   ]
        model.lb{i} = [throttle_min, steering_min, -inf,    -inf,       -inf,    -inf,    -inf,   0,                                -inf, -inf, -inf, 0   , -inf, -inf];
        model.ub{i} = [throttle_max, steering_max, +inf,    +inf,       +inf,    +inf,    +inf,   track_length+extended_distance ,  +inf, +inf, +inf, +inf, +inf, +inf];
    ...
    % Problem dimensions
    model.nvar(N-1) = prev_variables + n_input + additional_variables + n_state;
    model.neq(N-1) =  additional_variables + n_state; %progress, states
    model.nh(N-1) = 3; %input rate constraints 2
    ...
    % Simple Inequalities
    %               [d(t)          delta(t)      d(t-1)   delta(t-1)  vx(t-1)  vy(t-1)  r(t-1)  s(t)                              X     Y     Psi   vx    vy    r   ]
    model.lb{N-1} = [throttle_min, steering_min, -inf,    -inf,       -inf,    -inf,    -inf,   0,                                -inf, -inf, -inf, 0   , -inf, -inf];
    model.ub{N-1} = [throttle_max, steering_max, +inf,    +inf,       +inf,    +inf,    +inf,   track_length+extended_distance ,  +inf, +inf, +inf, +inf, +inf, +inf];
    ...
    % Problem dimensions
    model.nvar(N) = n_state + additional_variables; % states + progress
    model.nh(N) = 0;
    ...
    % Simple Inequalities
    %             [s(t)                             X    Y     Psi    vx   vy    r   ]
    model.lb{N} = [0,                              -inf, -inf, -inf, 0   , -inf, -inf];
    model.ub{N} = [track_length+extended_distance, +inf, +inf, +inf, +inf, +inf, +inf];
...
function f = objectiveInitial(z,p)
    ...
    f = 1/integrator_stepsize*beta_steer*(steering-steering_prev)^2 + 1/integrator_stepsize*beta_throttle*(throttle-throttle_prev)^2; 
end
% Middle
function f = objectiveMiddle(z,p)
    ...
    [g_x, g_y, dg_x, dg_y] = splineTrack(s+0.4);
    ...
    f = q_r*r^2 + alpha_CL*e_CL^2 + alpha_L*e_L^2 + 1/integrator_stepsize*beta_steer*(steering-steering_prev)^2 + 1/integrator_stepsize*beta_throttle*(throttle-throttle_prev)^2 + exp(q_v_max*(vel-v_max)) - lambda*vk*integrator_stepsize;
end
% End
function f = objectiveLast(z,p)
    ...
    f = q_r*r^2 + alpha_CL*e_CL^2 + alpha_L*e_L^2 + exp(q_v_max*(vel-v_max)) - lambda*vk*integrator_stepsize;
end
...
function h = ineqI(z,p)    
    ...
    h = [
         (u(1)-u_prev(1))/(1/integrator_stepsize*delta_throttle_max);       % rate throttle
         (u(2)-u_prev(2))/(1/integrator_stepsize*delta_steering_max);       % rate steering
         ];                    
end
% Middle
function h = ineqM(z,p)
    ...
    % Downforce Distribution
    F_downforce = 0.5 * rho * C_l * vx^2;

    Ff_downforce = F_downforce * downforce_front;
    Fr_downforce = F_downforce * downforce_rear;

    % Slip Angles
    alpha_f = u(2) - atan2(vy + r*l_f, vx);
    alpha_r = atan2((vy - r*l_r), vx);
    ...
    % NN correction
    NN_input = [u_prev(1) u_prev(2) vx_prev vy_prev r_prev; u(1) u(2) vx vy r];
    NN_correction = NN_flag * NNmodel(NN_input,p);
    
    friction_circle = ((F_x/m + NN_correction(1))^2 + (F_y/m + NN_correction(2))^2)/(mu*g)^2;

    velocity = sqrt(vx^2 + vy^2);
    friction_flag = velocity > lambda_blend_max;

    h = [
         (u(1)-u_prev(1))/(1/integrator_stepsize*delta_throttle_max);       % rate throttle
         (u(2)-u_prev(2))/(1/integrator_stepsize*delta_steering_max);       % rate steering
         friction_flag*friction_circle
         ];  
end
% End
