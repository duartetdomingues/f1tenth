...
% NN
LAYERS = 2; % Number of Decode/Encode Layers
...
if GENERATE_SOLVER
%     formulateMPCC_agnostic_noNN(STORE_MEX_DATA,diffCoeffs);
    formulateMPCC_agnostic(STORE_MEX_DATA, diffCoeffs);
%     formulateMPCC_agnostic_noNN_4Fy(STORE_MEX_DATA,diffCoeffs);
%     formulateMPCC_agnostic_noNN_4Fy_constraints(STORE_MEX_DATA,diffCoeffs);
%     formulateMPCC_agnostic_noNN_4Fy_cost(STORE_MEX_DATA,diffCoeffs);
end
...
%         X_val = fnval(X_spline,linspace(progresses(1),progresses(end),1000));
%         Y_val = fnval(Y_spline,linspace(progresses(1),progresses(end),1000));
%         close all
%         figure(1)
%         plot(X_val,Y_val);
%         hold on
...
%             problem.xinit = [output.z(3 + interval),output.z(4 + interval),output.z(3),output.z(4),output.z(14),output.z(15),output.z(16),0,output.z(11 + interval),output.z(12 + interval),output.z(13 + interval),output.z(14 + interval),output.z(15 + interval),output.z(16 + interval)]';
            fail_counter = 1;
            problem.xinit = [output.z(3 + interval*fail_counter),output.z(4 + interval*fail_counter),output.z(5 + interval*fail_counter),output.z(6 + interval*fail_counter),output.z(7 + interval*fail_counter),output.z(8 + interval*fail_counter),output.z(9 + interval*fail_counter),0,output.z(11 + interval*fail_counter),output.z(12 + interval*fail_counter),output.z(13 + interval*fail_counter),output.z(14 + interval*fail_counter),output.z(15 + interval*fail_counter),output.z(16 + interval*fail_counter)]';
            
        elseif fail_counter < 30
            fail_counter = fail_counter + 1;
            problem.xinit = [resizedZ_fail(3 + interval*fail_counter),resizedZ_fail(4 + interval*fail_counter),resizedZ_fail(5 + interval*fail_counter),resizedZ_fail(6 + interval*fail_counter),resizedZ_fail(7 + interval*fail_counter),resizedZ_fail(8 + interval*fail_counter),resizedZ_fail(9 + interval*fail_counter),0,resizedZ_fail(11 + interval*fail_counter),resizedZ_fail(12 + interval*fail_counter),resizedZ_fail(13 + interval*fail_counter),resizedZ_fail(14 + interval*fail_counter),resizedZ_fail(15 + interval*fail_counter),resizedZ_fail(16 + interval*fail_counter)]';
        else
            exit('Not good')
        end
            i = i + 1;
