...
% 
classdef solverMPCCBuildable < coder.ExternalDependency
    ...
    methods (Static)
        ...
        function updateBuildInfo(buildInfo, cfg)
            ...
            if(~strcmp(foldername, 'interface') || ~strcmp(solvername, 'solverMPCC'))
                solverpath = fullfile(buildablepath, 'solverMPCC');
            end
            solverInfo = struct();
            solverInfo.solvername = 'solverMPCC';
        ...
        function [output, exitflag, info] = forcesInitOutputsMatlab()
            ...
            outputs_z = coder.nullcopy(zeros(833, 1));
            outputs_zN_1 = coder.nullcopy(zeros(15, 1));
            outputs_zN = coder.nullcopy(zeros(11, 1));
            output = struct('z', outputs_z,...
                            'zN_1', outputs_zN_1,...
                            'zN', outputs_zN);
        ...
        function [output,exitflag,info] = forcesCallWithParams(params)
            [output,exitflag,info] = solverMPCCBuildable.forcesCall(params.x0, params.xinit, params.all_parameters, params.solver_timeout, params.num_of_threads);
        end

        function [output, exitflag, info] = forcesCall(x0, xinit, all_parameters, solver_timeout, num_of_threads)
            solvername = 'solverMPCC';

            
            params = struct('x0', double(x0),...
                            'xinit', double(xinit),...
                            'all_parameters', double(all_parameters),...
                            'solver_timeout', double(solver_timeout),...
                            'num_of_threads', uint32(num_of_threads));

            [output_c, exitflag_c, info_c] = solverMPCCBuildable.forcesInitOutputsC(); %#ok<ASGLU>
            ...
            [output, exitflag, info] = solverMPCCBuildable.forcesInitOutputsMatlab();
            ...
            output.z = cast(output_c.z, 'like', output.z);
            output.zN_1 = cast(output_c.zN_1, 'like', output.zN_1);
            output.zN = cast(output_c.zN, 'like', output.zN);
    ...
    methods (Static, Access = private)
        function [output, exitflag, info] = forcesInitOutputsC()
            ...
            outputs_z = coder.nullcopy(double(zeros(833, 1)));
            outputs_zN_1 = coder.nullcopy(double(zeros(15, 1)));
            outputs_zN = coder.nullcopy(double(zeros(11, 1)));
            output = struct('z', outputs_z,...
                            'zN_1', outputs_zN_1,...
                            'zN', outputs_zN);
