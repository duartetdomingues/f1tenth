function formulateMPCC_ICRA_v2(store_mex_data,differentailCoeffs) %TO DO - ADD ELLIPSE CONSTRAINT, add cost term regarding the slip angle in order to reduce aggressiveness (AMZ), use the derivative of the control input, incorporate lift force(downforce), add cone related constraints, look into the warm start, check for inequalities that depend on consecutive timesteps, might be necessary to take the last ones, tire models need a closer look, add the jacobian (might speed up), experiment with different solvers to see which one performs better, decouple the throttle from the problem and send that command to a PID controller like pure pursuit did
    ...
    model.lb{N} = [-inf,    0   ,   -inf,   -inf,  0,    -inf, -inf, -inf, 0   , -inf, -inf];
...
function f = objectiveInitial(z,p)
    ...
    f = beta_steer*z(2)^2 + beta_throttle*z(1)^2 + beta_epsilon * z(3)^2; 
end
% Middle
function f = objectiveMiddle(z,p)
    ...
    f = alpha_CL*e_CL^2 + alpha_L*e_L^2 + beta_steer*d_steering^2 + beta_throttle*d_throttle^2 - lambda*vk*integrator_stepsize + beta_epsilon * epsilon^2;
end
%N-1
function f = objectiveN_1(z,p)
   ...
   % Parameters
    ...
    f = alpha_CL*e_CL^2 + alpha_L*e_L^2 - lambda*vk*integrator_stepsize + beta_epsilon * epsilon^2;
end
% End
...
%     disp(p(end-2:end))
    ...
    delta_Fz_y_f = l_r/(l_r+l_f)/track_width*m*a_y*h_cog;
    delta_Fz_y_r = l_f/(l_r+l_f)/track_width*m*a_y*h_cog;
...
function h = ineqI(z,p)    
    ...
    h = [
         ...
         % (a_y/mu_y)^2 - (g)^2 - epsilon       % friction circle
         ];   
end
