...
function f = objectiveMiddle(z,p)
    ...
    r_max = 0.925*mu*g*approx_inv_vx;

    f = q_r*(r/r_max)^2 + alpha_CL*e_CL^2 + alpha_L*e_L^2 + beta_steer*d_steering^2 + beta_throttle*d_throttle^2 + exp(q_v_max*(vel-v_max)) - lambda*vk*integrator_stepsize;
end
%N-1
function f = objectiveN_1(z,p)
   ...
   % Parameters
    ...
    f = q_r*(r/r_max)^2 + alpha_CL*e_CL^2 + alpha_L*e_L^2 + exp(q_v_max*(vel-v_max)) - lambda*vk*integrator_stepsize;
end
% End
