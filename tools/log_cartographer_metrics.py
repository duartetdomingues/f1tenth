...
class CartographerMetricsLogger(Node):
    ...
    def save_map(self, filename="map_output.pbstream"):
        ...
        out_dir = f"cartographer_metrics_{timestamp}"
        ...
        try:
            map_filestem = os.path.join(out_dir, "map_output")
            subprocess.run([
            ...
            self.get_logger().info(f"Mapa convertido com sucesso para .pgm e .yaml em: {out_dir}")
    ...
    def process_metrics(self, response):
        ...
        lua_files = [
            "/home/desktop/f1tenth/src/cartographer_ros/cartographer_ros/configuration_files/backpack_2d.lua",
            "/home/desktop/f1tenth/src/cartographer/configuration_files/trajectory_builder_2d.lua"
        ]
        for lua_file in lua_files:
            if os.path.exists(lua_file):
                shutil.copy(lua_file, out_dir)

        # Save the CSV file in the directory
        filename = os.path.join(out_dir, "cartographer_metrics.csv")
        timestamp = datetime.now().strftime("%Y-%m-%d_%H-%M-%S")
