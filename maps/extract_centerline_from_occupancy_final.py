...
def remove_small_branches(skeleton, min_length=5):
    """ Remove small branches from the skeleton by analyzing its connectivity. """
    # Label connected components
    labeled_skeleton, num_features = cv2.connectedComponents(skeleton.astype(np.uint8))

    # Get unique labels of the components (ignore the background label 0)
    ...
    component_sizes = [np.sum(labeled_skeleton == label) for label in unique_labels]
    
    # Print component sizes for debugging
    print(f"Component sizes: {component_sizes}")

    # Create a new skeleton where small components are removed
    new_skeleton = np.zeros_like(skeleton)
    for label, size in zip(unique_labels, component_sizes):
        if size >= min_length:
            new_skeleton[labeled_skeleton == label] = 1

    # Debugging: Check how much of the skeleton remains
    print(f"Number of pixels remaining: {np.sum(new_skeleton)}")
...
if map_img is None:
    raise FileNotFoundError(f"Could not read the map image at path: {map_path}")

occupancy = (map_img < 220).astype(np.uint8)  # binariza

# === 2. Inverte para que 1 = livre (requerido por medial_axis)
free_space = 1 - occupancy

# === 4. Aplica morfologia para limpar o mapa
kernel = np.ones((20, 20), np.uint8)

# Fecha buracos pequenos (MORPH_CLOSE)
img_closed = cv2.morphologyEx(free_space, cv2.MORPH_CLOSE, kernel)

# Abre conexões finas e remove artefactos (MORPH_OPEN)
img_clean = cv2.morphologyEx(img_closed, cv2.MORPH_OPEN, kernel)

diff = np.sum(img_clean != free_space)
print(f"Número de pixels modificados: {diff}")

# === 5. Extrai o esqueleto (medial axis) limpo
skeleton, distance = medial_axis(img_clean, return_distance=True)

print("Número de pixels no esqueleto:", np.sum(skeleton))
...
plt.scatter(xs, ys, c='r', s=0.5, label="Skeleton points")
plt.scatter(xs[0], ys[0], c='blue', s=50, label="First point")
plt.axis("equal")
plt.legend()
plt.title("Skeleton extracted from occupancy map")
plt.show() 
...
for i, p in enumerate(points):
    dists, idxs = tree.query(p, k=20, distance_upper_bound=5.0)
    for j in idxs:
...
ordered_points = points[largest_cycle + [largest_cycle[0]]]  # fecha o loop


# === 6. Plot the largest cycle
plt.figure(figsize=(10, 6))
plt.imshow(occupancy, cmap='gray', origin='lower')
plt.scatter(ordered_points[:, 0], ordered_points[:, 1], c='red', s=1, label='Largest cycle')
plt.scatter(ordered_points[0, 0], ordered_points[0, 1], c='blue', s=50, label='Start point')
plt.legend()    
plt.title("Largest cycle extracted from skeleton")
plt.axis("equal")
plt.show()
...
def smooth_points(points, window_length=51, polyorder=3):
    """ Smooth the points using Savitzky-Golay filter to handle outliers. """
...
smoothed_points = smooth_points(ordered_points, window_length=250, polyorder=3)
